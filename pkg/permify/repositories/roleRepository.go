...
import (
	...
	"github.com/galexrt/arpanet/pkg/permify/collections"
	"github.com/galexrt/arpanet/pkg/permify/models"
	"github.com/galexrt/arpanet/pkg/permify/models/pivot"
	"github.com/galexrt/arpanet/pkg/permify/repositories/scopes"
)
...
func (repository *RoleRepository) GetRoleByID(ID uint) (role models.Role, err error) {
	err = repository.Database.First(&role, models.TablePrefix+"roles.id = ?", ID).Error
	return
}
...
func (repository *RoleRepository) GetRoleByIDWithPermissions(ID uint) (role models.Role, err error) {
	err = repository.Database.Preload(models.TablePrefix+"permissions").First(&role, models.TablePrefix+"roles.id = ?", ID).Error
	return
}
...
func (repository *RoleRepository) GetRoleByGuardName(guardName string) (role models.Role, err error) {
	err = repository.Database.Where(models.TablePrefix+"roles.guard_name = ?", guardName).First(&role).Error
	return
}
...
func (repository *RoleRepository) GetRoleByGuardNameWithPermissions(guardName string) (role models.Role, err error) {
	err = repository.Database.Preload(models.TablePrefix+"permissions").Where(models.TablePrefix+"roles.guard_name = ?", guardName).First(&role).Error
	return
}
...
func (repository *RoleRepository) GetRoles(IDs []uint) (roles collections.Role, err error) {
	err = repository.Database.Where(models.TablePrefix+"roles.id IN (?)", IDs).Find(&roles).Error
	return
}
...
func (repository *RoleRepository) GetRolesWithPermissions(IDs []uint) (roles collections.Role, err error) {
	err = repository.Database.Preload(models.TablePrefix+"permissions").Where(models.TablePrefix+"roles.id IN (?)", IDs).Find(&roles).Error
	return
}
...
func (repository *RoleRepository) GetRolesByGuardNames(guardNames []string) (roles collections.Role, err error) {
	err = repository.Database.Where(models.TablePrefix+"roles.guard_name IN (?)", guardNames).Find(&roles).Error
	return
}
...
func (repository *RoleRepository) GetRolesByGuardNamesWithPermissions(guardNames []string) (roles collections.Role, err error) {
	err = repository.Database.Preload(models.TablePrefix+"permissions").Where(models.TablePrefix+"roles.guard_name IN (?)", guardNames).Find(&roles).Error
	return
}
...
func (repository *RoleRepository) GetRoleIDs(pagination scopes.GormPager) (roleIDs []uint, totalCount int64, err error) {
	err = repository.Database.Model(&models.Role{}).Count(&totalCount).Scopes(repository.paginate(pagination)).Pluck(models.TablePrefix+"roles.id", &roleIDs).Error
	return
}
...
func (repository *RoleRepository) GetRoleIDsOfUser(userID uint, pagination scopes.GormPager) (roleIDs []uint, totalCount int64, err error) {
	err = repository.Database.Table(models.TablePrefix+"user_roles").Where(models.TablePrefix+"user_roles.user_id = ?", userID).Count(&totalCount).Scopes(repository.paginate(pagination)).Pluck(models.TablePrefix+"user_roles.role_id", &roleIDs).Error
	return
}
...
func (repository *RoleRepository) GetRoleIDsOfPermission(permissionID uint, pagination scopes.GormPager) (roleIDs []uint, totalCount int64, err error) {
	err = repository.Database.Table(models.TablePrefix+"role_permissions").Where(models.TablePrefix+"role_permissions.permission_id = ?", permissionID).Count(&totalCount).Scopes(repository.paginate(pagination)).Pluck(models.TablePrefix+"role_permissions.role_id", &roleIDs).Error
	return
}
...
func (repository *RoleRepository) Delete(role *models.Role) (err error) {
	return repository.Database.Transaction(func(tx *gorm.DB) error {
		if err := tx.Where(models.TablePrefix+"user_roles.role_id = ?", role.ID).Delete(&pivot.UserRoles{}).Error; err != nil {
			tx.Rollback()
			return err
		}
...
func (repository *RoleRepository) HasPermission(roles collections.Role, permission models.Permission) (b bool, err error) {
	var count int64
	err = repository.Database.Table(models.TablePrefix+"role_permissions").Where(models.TablePrefix+"role_permissions.role_id IN (?)", roles.IDs()).Where(models.TablePrefix+"role_permissions.permission_id = ?", permission.ID).Count(&count).Error
	return count > 0, err
}
...
func (repository *RoleRepository) HasAllPermissions(roles collections.Role, permissions collections.Permission) (b bool, err error) {
	var count int64
	err = repository.Database.Table(models.TablePrefix+"role_permissions").Where(models.TablePrefix+"role_permissions.role_id IN (?)", roles.IDs()).Where(models.TablePrefix+"role_permissions.permission_id IN (?)", permissions.IDs()).Count(&count).Error
