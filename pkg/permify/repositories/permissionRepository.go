...
func (repository *PermissionRepository) GetPermissionByID(ID uint) (permission models.Permission, err error) {
	err = repository.Database.First(&permission, models.TablePrefix+"permissions.id = ?", ID).Error
	return
}
...
func (repository *PermissionRepository) GetPermissionByGuardName(guardName string) (permission models.Permission, err error) {
	err = repository.Database.Where(models.TablePrefix+"permissions.guard_name = ?", guardName).First(&permission).Error
	return
}
...
func (repository *PermissionRepository) GetPermissions(IDs []uint) (permissions collections.Permission, err error) {
	err = repository.Database.Where(models.TablePrefix+"permissions.id IN (?)", IDs).Find(&permissions).Error
	return
}
...
func (repository *PermissionRepository) GetPermissionsByGuardNames(guardNames []string) (permissions collections.Permission, err error) {
	err = repository.Database.Where(models.TablePrefix+"permissions.guard_name IN (?)", guardNames).Find(&permissions).Error
	return
}
...
func (repository *PermissionRepository) GetPermissionIDs(pagination scopes.GormPager) (permissionIDs []uint, totalCount int64, err error) {
	err = repository.Database.Model(&models.Permission{}).Count(&totalCount).Scopes(repository.paginate(pagination)).Pluck(models.TablePrefix+"permissions.id", &permissionIDs).Error
	return
}
...
func (repository *PermissionRepository) GetDirectPermissionIDsOfUserByID(userID uint, pagination scopes.GormPager) (permissionIDs []uint, totalCount int64, err error) {
	err = repository.Database.Table(models.TablePrefix+"user_permissions").Where(models.TablePrefix+"user_permissions.user_id = ?", userID).Count(&totalCount).Scopes(repository.paginate(pagination)).Pluck(models.TablePrefix+"user_permissions.permission_id", &permissionIDs).Error
	return
}
...
func (repository *PermissionRepository) GetPermissionIDsOfRolesByIDs(roleIDs []uint, pagination scopes.GormPager) (permissionIDs []uint, totalCount int64, err error) {
	err = repository.Database.Table(models.TablePrefix+"role_permissions").Distinct(models.TablePrefix+"role_permissions.permission_id").Where(models.TablePrefix+"role_permissions.role_id IN (?)", roleIDs).Count(&totalCount).Scopes(repository.paginate(pagination)).Pluck(models.TablePrefix+"role_permissions.permission_id", &permissionIDs).Error
	return
}
...
func (repository *PermissionRepository) Delete(permission *models.Permission) (err error) {
	return repository.Database.Transaction(func(tx *gorm.DB) error {
		if err := tx.Where(models.TablePrefix+"user_permissions.permission_id = ?", permission.ID).Delete(&pivot.UserPermissions{}).Error; err != nil {
			tx.Rollback()
			return err
		}
