...
func (repository *UserRepository) ReplacePermissions(userID uint, permissions collections.Permission) error {
	return repository.Database.Transaction(func(tx *gorm.DB) error {
		if err := tx.Where(models.TablePrefix+"user_permissions.user_id = ?", userID).Delete(&pivot.UserPermissions{}).Error; err != nil {
			tx.Rollback()
			return err
		}
...
func (repository *UserRepository) ClearPermissions(userID uint) (err error) {
	return repository.Database.Where(models.TablePrefix+"user_permissions.user_id = ?", userID).Delete(&pivot.UserPermissions{}).Error
}
...
func (repository *UserRepository) ReplaceRoles(userID uint, roles collections.Role) error {
	return repository.Database.Transaction(func(tx *gorm.DB) error {
		if err := tx.Where(models.TablePrefix+"user_roles.user_id = ?", userID).Delete(&pivot.UserRoles{}).Error; err != nil {
			tx.Rollback()
			return err
		}
		var userRoles []pivot.UserRoles
...
func (repository *UserRepository) ClearRoles(userID uint) (err error) {
	return repository.Database.Where(models.TablePrefix+"user_roles.user_id = ?", userID).Delete(&pivot.UserRoles{}).Error
}
...
func (repository *UserRepository) HasRole(userID uint, role models.Role) (b bool, err error) {
	var count int64
	err = repository.Database.Table(models.TablePrefix+"user_roles").Where(models.TablePrefix+"user_roles.user_id = ?", userID).Where(models.TablePrefix+"user_roles.role_id = ?", role.ID).Count(&count).Error
	return count > 0, err
}
...
func (repository *UserRepository) HasAllRoles(userID uint, roles collections.Role) (b bool, err error) {
	var count int64
	err = repository.Database.Table(models.TablePrefix+"user_roles").Where(models.TablePrefix+"user_roles.user_id = ?", userID).Where(models.TablePrefix+"user_roles.role_id IN (?)", roles.IDs()).Count(&count).Error
...
func (repository *UserRepository) HasDirectPermission(userID uint, permission models.Permission) (b bool, err error) {
	var count int64
	err = repository.Database.Table(models.TablePrefix+"user_permissions").Where(models.TablePrefix+"user_permissions.user_id = ?", userID).Where(models.TablePrefix+"user_permissions.permission_id = ?", permission.ID).Count(&count).Error
	return count > 0, err
}
...
func (repository *UserRepository) HasAllDirectPermissions(userID uint, permissions collections.Permission) (b bool, err error) {
	var count int64
	err = repository.Database.Table(models.TablePrefix+"user_permissions").Where(models.TablePrefix+"user_permissions.user_id = ?", userID).Where(models.TablePrefix+"user_permissions.permission_id IN (?)", permissions.IDs()).Count(&count).Error
