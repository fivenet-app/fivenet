...
func TestStoreListQualificationsResultsUsesVisibilityCte(t *testing.T) {
	t.Parallel()
	...
	countQuery := regexp.QuoteMeta(`WITH user_subjects AS`) +
		...
		`(?s).*` + regexp.QuoteMeta(`COUNT(DISTINCT qualification_result.user_id) AS "data_count.total"`)
	mock.ExpectQuery(countQuery).
		WillReturnRows(sqlmock.NewRows([]string{"data_count.total"}).AddRow(int64(0)))

	pageSize := int64(10)
	resp, err := store.ListQualificationsResults(
		t.Context(),
		ListQualificationsResultsOptions{
...
func TestStoreListQualificationsResultsSuperuserBypassesVisibilityCte(t *testing.T) {
	t.Parallel()
