...
func TestStoreUpdateQualificationResultRejectsDeletedResult(t *testing.T) {
	t.Parallel()
	...
	err = store.UpdateQualificationResult(
		t.Context(),
		db,
		42,
		99,
...
func TestStoreRestoreQualificationResultRebuildsSuccessMap(t *testing.T) {
	t.Parallel()
	...
	mock.ExpectExec("(?s)UPDATE fivenet_qualifications_results SET deleted_at = NULL.*LIMIT \\?;").
		WithArgs(int64(99), int64(1)).
		WillReturnResult(sqlmock.NewResult(0, 1))
	...
	require.NoError(t, store.RestoreQualificationResult(t.Context(), db, 99, 42))
	require.NoError(t, mock.ExpectationsWereMet())
}
