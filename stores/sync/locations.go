...
func (s *Store) handleUserLocations(
	...
	if clearAll {
		condition := mysql.OR(tLocations.UserID.IS_NOT_NULL(), tLocations.UserID.IS_NULL())
		var count database.DataCount

		countStmt := tLocations.
			SELECT(mysql.COUNT(tLocations.UserID).AS("data_count.total")).
			FROM(tLocations).
			WHERE(condition)
	...
	if len(toDelete) > 0 {
		userIds := make([]mysql.Expression, 0, len(toDelete))
