...
func TestStoreViewAuditLogAppliesFiltersAndSort(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT audit_entry.id AS "audit_entry.id", audit_entry.created_at AS "audit_entry.created_at", audit_entry.user_id AS "audit_entry.user_id", audit_entry.user_job AS "audit_entry.user_job", audit_entry.account_id AS "audit_entry.account_id", audit_entry.target_user_id AS "audit_entry.target_user_id", audit_entry.service AS "audit_entry.service", audit_entry.method AS "audit_entry.method", audit_entry.action AS "audit_entry.action", audit_entry.result AS "audit_entry.result", audit_entry.meta AS "audit_entry.meta", audit_entry.data AS "audit_entry.data", user_short.id AS "user_short.id", user_short.identifier AS "user_short.identifier", user_short.job AS "user_short.job", user_short.job_grade AS "user_short.job_grade", user_short.firstname AS "user_short.firstname", user_short.lastname AS "user_short.lastname", user_short.dateofbirth AS "user_short.dateofbirth" FROM fivenet_audit_log AS audit_entry LEFT JOIN fivenet_user AS user_short ON`) + `(?s).*` + regexp.QuoteMeta(`ORDER BY audit_entry.service DESC`)).
