...
func (s *Server) checkIfUserHasAccessToTemplates(ctx context.Context, userId int32, job string, jobGrade int32, publicOk bool, access documents.ACCESS_LEVEL, templateIds ...uint64) (bool, error) {
	out, err := s.checkIfUserHasAccessToTemplateIDs(ctx, userId, job, jobGrade, publicOk, access, templateIds...)
	return len(out) == len(templateIds), err
}
