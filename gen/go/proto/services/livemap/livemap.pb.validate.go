...
func (m *StreamResponse) validate(all bool) error {
	...
	case *StreamResponse_UserDelete:
		...
		if all {
			switch v := interface{}(m.GetUserDelete()).(type) {
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, StreamResponseValidationError{
						field:  "UserDelete",
		...
		} else if v, ok := interface{}(m.GetUserDelete()).(interface{ Validate() error }); ok {
			if err := v.Validate(); err != nil {
				return StreamResponseValidationError{
					field:  "UserDelete",
					reason: "embedded message failed validation",
...
// ValidateAll checks the field values on UserDelete with the rules defined in
// the proto definition for this message. If any rules are violated, the
// result is a list of violation errors wrapped in UserDeleteMultiError, or
// nil if none found.
func (m *UserDelete) ValidateAll() error {
	return m.validate(true)
}

func (m *UserDelete) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	if len(errors) > 0 {
		return UserDeleteMultiError(errors)
	}

	return nil
}

// UserDeleteMultiError is an error wrapping multiple validation errors
// returned by UserDelete.ValidateAll() if the designated constraints aren't met.
type UserDeleteMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m UserDeleteMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m UserDeleteMultiError) AllErrors() []error { return m }
