...
func (m *CreateAccountRequest) validate(all bool) error {
	...
	if !_CreateAccountRequest_Username_Pattern.MatchString(m.GetUsername()) {
		err := CreateAccountRequestValidationError{
			field:  "Username",
			reason: "value does not match regex pattern \"^[0-9A-Za-zÄÖÜß_-]*$\"",
		}
		if !all {
			return err
		}
...
var _CreateAccountRequest_Username_Pattern = regexp.MustCompile("^[0-9A-Za-zÄÖÜß_-]*$")
...
var _LoginRequest_Username_Pattern = regexp.MustCompile("^[0-9A-Za-zÄÖÜß_-]*$")
