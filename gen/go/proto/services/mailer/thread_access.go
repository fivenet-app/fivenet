...
func (s *Server) checkIfUserHasAccessToThread(ctx context.Context, threadId uint64, userInfo *userinfo.UserInfo, access mailer.AccessLevel) (bool, error) {
	out, err := s.checkIfUserHasAccessToThreadIDs(ctx, userInfo, access, threadId)
	return len(out) > 0, err
}

func (s *Server) checkIfUserHasAccessToThreads(ctx context.Context, userInfo *userinfo.UserInfo, access mailer.AccessLevel, threadIds ...uint64) (bool, error) {
	out, err := s.checkIfUserHasAccessToThreadIDs(ctx, userInfo, access, threadIds...)
	return len(out) == len(threadIds), err
}

func (s *Server) checkIfUserHasAccessToThreadIDs(ctx context.Context, userInfo *userinfo.UserInfo, access mailer.AccessLevel, threadIds ...uint64) ([]uint64, error) {
	if len(threadIds) == 0 {
