...
func (s *Server) getThreadAccess(ctx context.Context, threadId uint64) (*mailer.ThreadAccess, error) {
	...
	var jobAccess []*mailer.ThreadJobAccess
	...
	var userAccess []*mailer.ThreadUserAccess
	...
	return &mailer.ThreadAccess{
		Jobs:  jobAccess,
		Users: userAccess,
	}, nil
}

func (s *Server) handleThreadAccessChanges(ctx context.Context, tx qrm.DB, threadId uint64, access *mailer.ThreadAccess) (*mailer.ThreadAccess, error) {
...
func (s *Server) compareThreadAccess(current, in *mailer.ThreadAccess) (toCreate *mailer.ThreadAccess, toUpdate *mailer.ThreadAccess, toDelete *mailer.ThreadAccess) {
	toCreate = &mailer.ThreadAccess{}
	toUpdate = &mailer.ThreadAccess{}
	toDelete = &mailer.ThreadAccess{}

	if current == nil || (len(current.Jobs) == 0 && len(current.Users) == 0) {
		return in, toUpdate, toDelete
	}

	slices.SortFunc(current.Jobs, func(a, b *mailer.ThreadJobAccess) int {
	...
	if len(current.Jobs) == 0 {
		...
		for _, cj := range current.Jobs {
			var found *mailer.ThreadJobAccess
			var foundIdx int
	...
	if len(current.Users) == 0 {
		...
		for _, cj := range current.Users {
			var found *mailer.ThreadUserAccess
			var foundIdx int
...
func (s *Server) createThreadAccess(ctx context.Context, tx qrm.DB, threadId uint64, access *mailer.ThreadAccess) error {
	if access == nil {
		return nil
	}
...
func (s *Server) updateThreadAccess(ctx context.Context, tx qrm.DB, threadId uint64, access *mailer.ThreadAccess) error {
	if access == nil {
		return nil
	}
...
func (s *Server) deleteThreadAccess(ctx context.Context, tx qrm.DB, threadId uint64, access *mailer.ThreadAccess) error {
	if access == nil {
		return nil
	}
