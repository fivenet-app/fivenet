...
// ValidateAll checks the field values on Guild with the rules defined in the
// proto definition for this message. If any rules are violated, the result is
// a list of violation errors wrapped in GuildMultiError, or nil if none found.
func (m *Guild) ValidateAll() error {
	return m.validate(true)
}

func (m *Guild) validate(all bool) error {
	if m == nil {
		return nil
	}
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, GuildValidationError{
						field:  "CreatedAt",
	...
	if len(errors) > 0 {
		return GuildMultiError(errors)
	}

	return nil
}

// GuildMultiError is an error wrapping multiple validation errors returned by
// Guild.ValidateAll() if the designated constraints aren't met.
type GuildMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m GuildMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m GuildMultiError) AllErrors() []error { return m }
