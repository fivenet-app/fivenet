// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.10
// 	protoc        (unknown)
// source: resources/wiki/activity/activity.proto

//go:build protoopaque

package wikiactivity

import (
	_ "github.com/fivenet-app/fivenet/v2026/gen/go/proto/codegen/dbscanner"
	content "github.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/common/content"
	timestamp "github.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/timestamp"
	short "github.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/users/short"
	access "github.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/wiki/access"
	_ "github.com/srikrsna/protoc-gen-gotag/tagger"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type PageActivityType int32

const (
	PageActivityType_PAGE_ACTIVITY_TYPE_UNSPECIFIED PageActivityType = 0
	// Base
	PageActivityType_PAGE_ACTIVITY_TYPE_CREATED        PageActivityType = 1
	PageActivityType_PAGE_ACTIVITY_TYPE_UPDATED        PageActivityType = 2
	PageActivityType_PAGE_ACTIVITY_TYPE_ACCESS_UPDATED PageActivityType = 3
	PageActivityType_PAGE_ACTIVITY_TYPE_OWNER_CHANGED  PageActivityType = 4
	PageActivityType_PAGE_ACTIVITY_TYPE_DELETED        PageActivityType = 5
	PageActivityType_PAGE_ACTIVITY_TYPE_DRAFT_TOGGLED  PageActivityType = 6
)

// Enum value maps for PageActivityType.
var (
	PageActivityType_name = map[int32]string{
		0: "PAGE_ACTIVITY_TYPE_UNSPECIFIED",
		1: "PAGE_ACTIVITY_TYPE_CREATED",
		2: "PAGE_ACTIVITY_TYPE_UPDATED",
		3: "PAGE_ACTIVITY_TYPE_ACCESS_UPDATED",
		4: "PAGE_ACTIVITY_TYPE_OWNER_CHANGED",
		5: "PAGE_ACTIVITY_TYPE_DELETED",
		6: "PAGE_ACTIVITY_TYPE_DRAFT_TOGGLED",
	}
	PageActivityType_value = map[string]int32{
		"PAGE_ACTIVITY_TYPE_UNSPECIFIED":    0,
		"PAGE_ACTIVITY_TYPE_CREATED":        1,
		"PAGE_ACTIVITY_TYPE_UPDATED":        2,
		"PAGE_ACTIVITY_TYPE_ACCESS_UPDATED": 3,
		"PAGE_ACTIVITY_TYPE_OWNER_CHANGED":  4,
		"PAGE_ACTIVITY_TYPE_DELETED":        5,
		"PAGE_ACTIVITY_TYPE_DRAFT_TOGGLED":  6,
	}
)

func (x PageActivityType) Enum() *PageActivityType {
	p := new(PageActivityType)
	*p = x
	return p
}

func (x PageActivityType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (PageActivityType) Descriptor() protoreflect.EnumDescriptor {
	return file_resources_wiki_activity_activity_proto_enumTypes[0].Descriptor()
}

func (PageActivityType) Type() protoreflect.EnumType {
	return &file_resources_wiki_activity_activity_proto_enumTypes[0]
}

func (x PageActivityType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

type PageActivity struct {
	state                      protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Id              int64                  `protobuf:"varint,1,opt,name=id,proto3"`
	xxx_hidden_CreatedAt       *timestamp.Timestamp   `protobuf:"bytes,2,opt,name=created_at,json=createdAt,proto3"`
	xxx_hidden_PageId          int64                  `protobuf:"varint,3,opt,name=page_id,json=pageId,proto3"`
	xxx_hidden_ActivityType    PageActivityType       `protobuf:"varint,4,opt,name=activity_type,json=activityType,proto3,enum=resources.wiki.activity.PageActivityType"`
	xxx_hidden_CreatorId       int32                  `protobuf:"varint,5,opt,name=creator_id,json=creatorId,proto3,oneof"`
	xxx_hidden_Creator         *short.UserShort       `protobuf:"bytes,6,opt,name=creator,proto3,oneof"`
	xxx_hidden_CreatorJob      string                 `protobuf:"bytes,7,opt,name=creator_job,json=creatorJob,proto3"`
	xxx_hidden_CreatorJobLabel *string                `protobuf:"bytes,8,opt,name=creator_job_label,json=creatorJobLabel,proto3,oneof"`
	xxx_hidden_Reason          *string                `protobuf:"bytes,9,opt,name=reason,proto3,oneof"`
	xxx_hidden_Data            *PageActivityData      `protobuf:"bytes,10,opt,name=data,proto3"`
	XXX_raceDetectHookData     protoimpl.RaceDetectHookData
	XXX_presence               [1]uint32
	unknownFields              protoimpl.UnknownFields
	sizeCache                  protoimpl.SizeCache
}

func (x *PageActivity) Reset() {
	*x = PageActivity{}
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PageActivity) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PageActivity) ProtoMessage() {}

func (x *PageActivity) ProtoReflect() protoreflect.Message {
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *PageActivity) GetId() int64 {
	if x != nil {
		return x.xxx_hidden_Id
	}
	return 0
}

func (x *PageActivity) GetCreatedAt() *timestamp.Timestamp {
	if x != nil {
		return x.xxx_hidden_CreatedAt
	}
	return nil
}

func (x *PageActivity) GetPageId() int64 {
	if x != nil {
		return x.xxx_hidden_PageId
	}
	return 0
}

func (x *PageActivity) GetActivityType() PageActivityType {
	if x != nil {
		return x.xxx_hidden_ActivityType
	}
	return PageActivityType_PAGE_ACTIVITY_TYPE_UNSPECIFIED
}

func (x *PageActivity) GetCreatorId() int32 {
	if x != nil {
		return x.xxx_hidden_CreatorId
	}
	return 0
}

func (x *PageActivity) GetCreator() *short.UserShort {
	if x != nil {
		return x.xxx_hidden_Creator
	}
	return nil
}

func (x *PageActivity) GetCreatorJob() string {
	if x != nil {
		return x.xxx_hidden_CreatorJob
	}
	return ""
}

func (x *PageActivity) GetCreatorJobLabel() string {
	if x != nil {
		if x.xxx_hidden_CreatorJobLabel != nil {
			return *x.xxx_hidden_CreatorJobLabel
		}
		return ""
	}
	return ""
}

func (x *PageActivity) GetReason() string {
	if x != nil {
		if x.xxx_hidden_Reason != nil {
			return *x.xxx_hidden_Reason
		}
		return ""
	}
	return ""
}

func (x *PageActivity) GetData() *PageActivityData {
	if x != nil {
		return x.xxx_hidden_Data
	}
	return nil
}

func (x *PageActivity) SetId(v int64) {
	x.xxx_hidden_Id = v
}

func (x *PageActivity) SetCreatedAt(v *timestamp.Timestamp) {
	x.xxx_hidden_CreatedAt = v
}

func (x *PageActivity) SetPageId(v int64) {
	x.xxx_hidden_PageId = v
}

func (x *PageActivity) SetActivityType(v PageActivityType) {
	x.xxx_hidden_ActivityType = v
}

func (x *PageActivity) SetCreatorId(v int32) {
	x.xxx_hidden_CreatorId = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 4, 10)
}

func (x *PageActivity) SetCreator(v *short.UserShort) {
	x.xxx_hidden_Creator = v
}

func (x *PageActivity) SetCreatorJob(v string) {
	x.xxx_hidden_CreatorJob = v
}

func (x *PageActivity) SetCreatorJobLabel(v string) {
	x.xxx_hidden_CreatorJobLabel = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 7, 10)
}

func (x *PageActivity) SetReason(v string) {
	x.xxx_hidden_Reason = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 8, 10)
}

func (x *PageActivity) SetData(v *PageActivityData) {
	x.xxx_hidden_Data = v
}

func (x *PageActivity) HasCreatedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_CreatedAt != nil
}

func (x *PageActivity) HasCreatorId() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 4)
}

func (x *PageActivity) HasCreator() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Creator != nil
}

func (x *PageActivity) HasCreatorJobLabel() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 7)
}

func (x *PageActivity) HasReason() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 8)
}

func (x *PageActivity) HasData() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Data != nil
}

func (x *PageActivity) ClearCreatedAt() {
	x.xxx_hidden_CreatedAt = nil
}

func (x *PageActivity) ClearCreatorId() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 4)
	x.xxx_hidden_CreatorId = 0
}

func (x *PageActivity) ClearCreator() {
	x.xxx_hidden_Creator = nil
}

func (x *PageActivity) ClearCreatorJobLabel() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 7)
	x.xxx_hidden_CreatorJobLabel = nil
}

func (x *PageActivity) ClearReason() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 8)
	x.xxx_hidden_Reason = nil
}

func (x *PageActivity) ClearData() {
	x.xxx_hidden_Data = nil
}

type PageActivity_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Id              int64
	CreatedAt       *timestamp.Timestamp
	PageId          int64
	ActivityType    PageActivityType
	CreatorId       *int32
	Creator         *short.UserShort
	CreatorJob      string
	CreatorJobLabel *string
	Reason          *string
	Data            *PageActivityData
}

func (b0 PageActivity_builder) Build() *PageActivity {
	m0 := &PageActivity{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Id = b.Id
	x.xxx_hidden_CreatedAt = b.CreatedAt
	x.xxx_hidden_PageId = b.PageId
	x.xxx_hidden_ActivityType = b.ActivityType
	if b.CreatorId != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 4, 10)
		x.xxx_hidden_CreatorId = *b.CreatorId
	}
	x.xxx_hidden_Creator = b.Creator
	x.xxx_hidden_CreatorJob = b.CreatorJob
	if b.CreatorJobLabel != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 7, 10)
		x.xxx_hidden_CreatorJobLabel = b.CreatorJobLabel
	}
	if b.Reason != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 8, 10)
		x.xxx_hidden_Reason = b.Reason
	}
	x.xxx_hidden_Data = b.Data
	return m0
}

type PageActivityData struct {
	state           protoimpl.MessageState  `protogen:"opaque.v1"`
	xxx_hidden_Data isPageActivityData_Data `protobuf_oneof:"data"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *PageActivityData) Reset() {
	*x = PageActivityData{}
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PageActivityData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PageActivityData) ProtoMessage() {}

func (x *PageActivityData) ProtoReflect() protoreflect.Message {
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *PageActivityData) GetUpdated() *PageUpdated {
	if x != nil {
		if x, ok := x.xxx_hidden_Data.(*pageActivityData_Updated); ok {
			return x.Updated
		}
	}
	return nil
}

func (x *PageActivityData) GetAccessUpdated() *PageAccessUpdated {
	if x != nil {
		if x, ok := x.xxx_hidden_Data.(*pageActivityData_AccessUpdated); ok {
			return x.AccessUpdated
		}
	}
	return nil
}

func (x *PageActivityData) SetUpdated(v *PageUpdated) {
	if v == nil {
		x.xxx_hidden_Data = nil
		return
	}
	x.xxx_hidden_Data = &pageActivityData_Updated{v}
}

func (x *PageActivityData) SetAccessUpdated(v *PageAccessUpdated) {
	if v == nil {
		x.xxx_hidden_Data = nil
		return
	}
	x.xxx_hidden_Data = &pageActivityData_AccessUpdated{v}
}

func (x *PageActivityData) HasData() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Data != nil
}

func (x *PageActivityData) HasUpdated() bool {
	if x == nil {
		return false
	}
	_, ok := x.xxx_hidden_Data.(*pageActivityData_Updated)
	return ok
}

func (x *PageActivityData) HasAccessUpdated() bool {
	if x == nil {
		return false
	}
	_, ok := x.xxx_hidden_Data.(*pageActivityData_AccessUpdated)
	return ok
}

func (x *PageActivityData) ClearData() {
	x.xxx_hidden_Data = nil
}

func (x *PageActivityData) ClearUpdated() {
	if _, ok := x.xxx_hidden_Data.(*pageActivityData_Updated); ok {
		x.xxx_hidden_Data = nil
	}
}

func (x *PageActivityData) ClearAccessUpdated() {
	if _, ok := x.xxx_hidden_Data.(*pageActivityData_AccessUpdated); ok {
		x.xxx_hidden_Data = nil
	}
}

const PageActivityData_Data_not_set_case case_PageActivityData_Data = 0
const PageActivityData_Updated_case case_PageActivityData_Data = 1
const PageActivityData_AccessUpdated_case case_PageActivityData_Data = 2

func (x *PageActivityData) WhichData() case_PageActivityData_Data {
	if x == nil {
		return PageActivityData_Data_not_set_case
	}
	switch x.xxx_hidden_Data.(type) {
	case *pageActivityData_Updated:
		return PageActivityData_Updated_case
	case *pageActivityData_AccessUpdated:
		return PageActivityData_AccessUpdated_case
	default:
		return PageActivityData_Data_not_set_case
	}
}

type PageActivityData_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Fields of oneof xxx_hidden_Data:
	Updated       *PageUpdated
	AccessUpdated *PageAccessUpdated
	// -- end of xxx_hidden_Data
}

func (b0 PageActivityData_builder) Build() *PageActivityData {
	m0 := &PageActivityData{}
	b, x := &b0, m0
	_, _ = b, x
	if b.Updated != nil {
		x.xxx_hidden_Data = &pageActivityData_Updated{b.Updated}
	}
	if b.AccessUpdated != nil {
		x.xxx_hidden_Data = &pageActivityData_AccessUpdated{b.AccessUpdated}
	}
	return m0
}

type case_PageActivityData_Data protoreflect.FieldNumber

func (x case_PageActivityData_Data) String() string {
	md := file_resources_wiki_activity_activity_proto_msgTypes[1].Descriptor()
	if x == 0 {
		return "not set"
	}
	return protoimpl.X.MessageFieldStringOf(md, protoreflect.FieldNumber(x))
}

type isPageActivityData_Data interface {
	isPageActivityData_Data()
}

type pageActivityData_Updated struct {
	Updated *PageUpdated `protobuf:"bytes,1,opt,name=updated,proto3,oneof"`
}

type pageActivityData_AccessUpdated struct {
	AccessUpdated *PageAccessUpdated `protobuf:"bytes,2,opt,name=access_updated,json=accessUpdated,proto3,oneof"`
}

func (*pageActivityData_Updated) isPageActivityData_Data() {}

func (*pageActivityData_AccessUpdated) isPageActivityData_Data() {}

type PageUpdated struct {
	state                       protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_TitleDiff        *string                `protobuf:"bytes,1,opt,name=title_diff,json=titleDiff,proto3,oneof"`
	xxx_hidden_TitleCdiff       *content.ContentDiff   `protobuf:"bytes,5,opt,name=title_cdiff,json=titleCdiff,proto3,oneof"`
	xxx_hidden_DescriptionDiff  *string                `protobuf:"bytes,2,opt,name=description_diff,json=descriptionDiff,proto3,oneof"`
	xxx_hidden_DescriptionCdiff *content.ContentDiff   `protobuf:"bytes,6,opt,name=description_cdiff,json=descriptionCdiff,proto3,oneof"`
	xxx_hidden_ContentDiff      *string                `protobuf:"bytes,3,opt,name=content_diff,json=contentDiff,proto3,oneof"`
	xxx_hidden_ContentCdiff     *content.ContentDiff   `protobuf:"bytes,7,opt,name=content_cdiff,json=contentCdiff,proto3,oneof"`
	xxx_hidden_FilesChange      *PageFilesChange       `protobuf:"bytes,4,opt,name=files_change,json=filesChange,proto3,oneof"`
	XXX_raceDetectHookData      protoimpl.RaceDetectHookData
	XXX_presence                [1]uint32
	unknownFields               protoimpl.UnknownFields
	sizeCache                   protoimpl.SizeCache
}

func (x *PageUpdated) Reset() {
	*x = PageUpdated{}
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PageUpdated) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PageUpdated) ProtoMessage() {}

func (x *PageUpdated) ProtoReflect() protoreflect.Message {
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *PageUpdated) GetTitleDiff() string {
	if x != nil {
		if x.xxx_hidden_TitleDiff != nil {
			return *x.xxx_hidden_TitleDiff
		}
		return ""
	}
	return ""
}

func (x *PageUpdated) GetTitleCdiff() *content.ContentDiff {
	if x != nil {
		return x.xxx_hidden_TitleCdiff
	}
	return nil
}

func (x *PageUpdated) GetDescriptionDiff() string {
	if x != nil {
		if x.xxx_hidden_DescriptionDiff != nil {
			return *x.xxx_hidden_DescriptionDiff
		}
		return ""
	}
	return ""
}

func (x *PageUpdated) GetDescriptionCdiff() *content.ContentDiff {
	if x != nil {
		return x.xxx_hidden_DescriptionCdiff
	}
	return nil
}

func (x *PageUpdated) GetContentDiff() string {
	if x != nil {
		if x.xxx_hidden_ContentDiff != nil {
			return *x.xxx_hidden_ContentDiff
		}
		return ""
	}
	return ""
}

func (x *PageUpdated) GetContentCdiff() *content.ContentDiff {
	if x != nil {
		return x.xxx_hidden_ContentCdiff
	}
	return nil
}

func (x *PageUpdated) GetFilesChange() *PageFilesChange {
	if x != nil {
		return x.xxx_hidden_FilesChange
	}
	return nil
}

func (x *PageUpdated) SetTitleDiff(v string) {
	x.xxx_hidden_TitleDiff = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 0, 7)
}

func (x *PageUpdated) SetTitleCdiff(v *content.ContentDiff) {
	x.xxx_hidden_TitleCdiff = v
}

func (x *PageUpdated) SetDescriptionDiff(v string) {
	x.xxx_hidden_DescriptionDiff = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 2, 7)
}

func (x *PageUpdated) SetDescriptionCdiff(v *content.ContentDiff) {
	x.xxx_hidden_DescriptionCdiff = v
}

func (x *PageUpdated) SetContentDiff(v string) {
	x.xxx_hidden_ContentDiff = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 4, 7)
}

func (x *PageUpdated) SetContentCdiff(v *content.ContentDiff) {
	x.xxx_hidden_ContentCdiff = v
}

func (x *PageUpdated) SetFilesChange(v *PageFilesChange) {
	x.xxx_hidden_FilesChange = v
}

func (x *PageUpdated) HasTitleDiff() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 0)
}

func (x *PageUpdated) HasTitleCdiff() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_TitleCdiff != nil
}

func (x *PageUpdated) HasDescriptionDiff() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 2)
}

func (x *PageUpdated) HasDescriptionCdiff() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_DescriptionCdiff != nil
}

func (x *PageUpdated) HasContentDiff() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 4)
}

func (x *PageUpdated) HasContentCdiff() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_ContentCdiff != nil
}

func (x *PageUpdated) HasFilesChange() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_FilesChange != nil
}

func (x *PageUpdated) ClearTitleDiff() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 0)
	x.xxx_hidden_TitleDiff = nil
}

func (x *PageUpdated) ClearTitleCdiff() {
	x.xxx_hidden_TitleCdiff = nil
}

func (x *PageUpdated) ClearDescriptionDiff() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 2)
	x.xxx_hidden_DescriptionDiff = nil
}

func (x *PageUpdated) ClearDescriptionCdiff() {
	x.xxx_hidden_DescriptionCdiff = nil
}

func (x *PageUpdated) ClearContentDiff() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 4)
	x.xxx_hidden_ContentDiff = nil
}

func (x *PageUpdated) ClearContentCdiff() {
	x.xxx_hidden_ContentCdiff = nil
}

func (x *PageUpdated) ClearFilesChange() {
	x.xxx_hidden_FilesChange = nil
}

type PageUpdated_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	TitleDiff        *string
	TitleCdiff       *content.ContentDiff
	DescriptionDiff  *string
	DescriptionCdiff *content.ContentDiff
	ContentDiff      *string
	ContentCdiff     *content.ContentDiff
	FilesChange      *PageFilesChange
}

func (b0 PageUpdated_builder) Build() *PageUpdated {
	m0 := &PageUpdated{}
	b, x := &b0, m0
	_, _ = b, x
	if b.TitleDiff != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 0, 7)
		x.xxx_hidden_TitleDiff = b.TitleDiff
	}
	x.xxx_hidden_TitleCdiff = b.TitleCdiff
	if b.DescriptionDiff != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 2, 7)
		x.xxx_hidden_DescriptionDiff = b.DescriptionDiff
	}
	x.xxx_hidden_DescriptionCdiff = b.DescriptionCdiff
	if b.ContentDiff != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 4, 7)
		x.xxx_hidden_ContentDiff = b.ContentDiff
	}
	x.xxx_hidden_ContentCdiff = b.ContentCdiff
	x.xxx_hidden_FilesChange = b.FilesChange
	return m0
}

type PageFilesChange struct {
	state              protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Added   int64                  `protobuf:"varint,1,opt,name=added,proto3"`
	xxx_hidden_Deleted int64                  `protobuf:"varint,2,opt,name=deleted,proto3"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *PageFilesChange) Reset() {
	*x = PageFilesChange{}
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PageFilesChange) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PageFilesChange) ProtoMessage() {}

func (x *PageFilesChange) ProtoReflect() protoreflect.Message {
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *PageFilesChange) GetAdded() int64 {
	if x != nil {
		return x.xxx_hidden_Added
	}
	return 0
}

func (x *PageFilesChange) GetDeleted() int64 {
	if x != nil {
		return x.xxx_hidden_Deleted
	}
	return 0
}

func (x *PageFilesChange) SetAdded(v int64) {
	x.xxx_hidden_Added = v
}

func (x *PageFilesChange) SetDeleted(v int64) {
	x.xxx_hidden_Deleted = v
}

type PageFilesChange_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Added   int64
	Deleted int64
}

func (b0 PageFilesChange_builder) Build() *PageFilesChange {
	m0 := &PageFilesChange{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Added = b.Added
	x.xxx_hidden_Deleted = b.Deleted
	return m0
}

type PageAccessUpdated struct {
	state            protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Jobs  *PageAccessJobsDiff    `protobuf:"bytes,1,opt,name=jobs,proto3"`
	xxx_hidden_Users *PageAccessUsersDiff   `protobuf:"bytes,2,opt,name=users,proto3"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *PageAccessUpdated) Reset() {
	*x = PageAccessUpdated{}
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PageAccessUpdated) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PageAccessUpdated) ProtoMessage() {}

func (x *PageAccessUpdated) ProtoReflect() protoreflect.Message {
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *PageAccessUpdated) GetJobs() *PageAccessJobsDiff {
	if x != nil {
		return x.xxx_hidden_Jobs
	}
	return nil
}

func (x *PageAccessUpdated) GetUsers() *PageAccessUsersDiff {
	if x != nil {
		return x.xxx_hidden_Users
	}
	return nil
}

func (x *PageAccessUpdated) SetJobs(v *PageAccessJobsDiff) {
	x.xxx_hidden_Jobs = v
}

func (x *PageAccessUpdated) SetUsers(v *PageAccessUsersDiff) {
	x.xxx_hidden_Users = v
}

func (x *PageAccessUpdated) HasJobs() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Jobs != nil
}

func (x *PageAccessUpdated) HasUsers() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Users != nil
}

func (x *PageAccessUpdated) ClearJobs() {
	x.xxx_hidden_Jobs = nil
}

func (x *PageAccessUpdated) ClearUsers() {
	x.xxx_hidden_Users = nil
}

type PageAccessUpdated_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Jobs  *PageAccessJobsDiff
	Users *PageAccessUsersDiff
}

func (b0 PageAccessUpdated_builder) Build() *PageAccessUpdated {
	m0 := &PageAccessUpdated{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Jobs = b.Jobs
	x.xxx_hidden_Users = b.Users
	return m0
}

type PageAccessJobsDiff struct {
	state               protoimpl.MessageState   `protogen:"opaque.v1"`
	xxx_hidden_ToCreate *[]*access.PageJobAccess `protobuf:"bytes,1,rep,name=to_create,json=toCreate,proto3"`
	xxx_hidden_ToUpdate *[]*access.PageJobAccess `protobuf:"bytes,2,rep,name=to_update,json=toUpdate,proto3"`
	xxx_hidden_ToDelete *[]*access.PageJobAccess `protobuf:"bytes,3,rep,name=to_delete,json=toDelete,proto3"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *PageAccessJobsDiff) Reset() {
	*x = PageAccessJobsDiff{}
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PageAccessJobsDiff) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PageAccessJobsDiff) ProtoMessage() {}

func (x *PageAccessJobsDiff) ProtoReflect() protoreflect.Message {
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *PageAccessJobsDiff) GetToCreate() []*access.PageJobAccess {
	if x != nil {
		if x.xxx_hidden_ToCreate != nil {
			return *x.xxx_hidden_ToCreate
		}
	}
	return nil
}

func (x *PageAccessJobsDiff) GetToUpdate() []*access.PageJobAccess {
	if x != nil {
		if x.xxx_hidden_ToUpdate != nil {
			return *x.xxx_hidden_ToUpdate
		}
	}
	return nil
}

func (x *PageAccessJobsDiff) GetToDelete() []*access.PageJobAccess {
	if x != nil {
		if x.xxx_hidden_ToDelete != nil {
			return *x.xxx_hidden_ToDelete
		}
	}
	return nil
}

func (x *PageAccessJobsDiff) SetToCreate(v []*access.PageJobAccess) {
	x.xxx_hidden_ToCreate = &v
}

func (x *PageAccessJobsDiff) SetToUpdate(v []*access.PageJobAccess) {
	x.xxx_hidden_ToUpdate = &v
}

func (x *PageAccessJobsDiff) SetToDelete(v []*access.PageJobAccess) {
	x.xxx_hidden_ToDelete = &v
}

type PageAccessJobsDiff_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	ToCreate []*access.PageJobAccess
	ToUpdate []*access.PageJobAccess
	ToDelete []*access.PageJobAccess
}

func (b0 PageAccessJobsDiff_builder) Build() *PageAccessJobsDiff {
	m0 := &PageAccessJobsDiff{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_ToCreate = &b.ToCreate
	x.xxx_hidden_ToUpdate = &b.ToUpdate
	x.xxx_hidden_ToDelete = &b.ToDelete
	return m0
}

type PageAccessUsersDiff struct {
	state               protoimpl.MessageState    `protogen:"opaque.v1"`
	xxx_hidden_ToCreate *[]*access.PageUserAccess `protobuf:"bytes,1,rep,name=to_create,json=toCreate,proto3"`
	xxx_hidden_ToUpdate *[]*access.PageUserAccess `protobuf:"bytes,2,rep,name=to_update,json=toUpdate,proto3"`
	xxx_hidden_ToDelete *[]*access.PageUserAccess `protobuf:"bytes,3,rep,name=to_delete,json=toDelete,proto3"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *PageAccessUsersDiff) Reset() {
	*x = PageAccessUsersDiff{}
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PageAccessUsersDiff) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PageAccessUsersDiff) ProtoMessage() {}

func (x *PageAccessUsersDiff) ProtoReflect() protoreflect.Message {
	mi := &file_resources_wiki_activity_activity_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *PageAccessUsersDiff) GetToCreate() []*access.PageUserAccess {
	if x != nil {
		if x.xxx_hidden_ToCreate != nil {
			return *x.xxx_hidden_ToCreate
		}
	}
	return nil
}

func (x *PageAccessUsersDiff) GetToUpdate() []*access.PageUserAccess {
	if x != nil {
		if x.xxx_hidden_ToUpdate != nil {
			return *x.xxx_hidden_ToUpdate
		}
	}
	return nil
}

func (x *PageAccessUsersDiff) GetToDelete() []*access.PageUserAccess {
	if x != nil {
		if x.xxx_hidden_ToDelete != nil {
			return *x.xxx_hidden_ToDelete
		}
	}
	return nil
}

func (x *PageAccessUsersDiff) SetToCreate(v []*access.PageUserAccess) {
	x.xxx_hidden_ToCreate = &v
}

func (x *PageAccessUsersDiff) SetToUpdate(v []*access.PageUserAccess) {
	x.xxx_hidden_ToUpdate = &v
}

func (x *PageAccessUsersDiff) SetToDelete(v []*access.PageUserAccess) {
	x.xxx_hidden_ToDelete = &v
}

type PageAccessUsersDiff_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	ToCreate []*access.PageUserAccess
	ToUpdate []*access.PageUserAccess
	ToDelete []*access.PageUserAccess
}

func (b0 PageAccessUsersDiff_builder) Build() *PageAccessUsersDiff {
	m0 := &PageAccessUsersDiff{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_ToCreate = &b.ToCreate
	x.xxx_hidden_ToUpdate = &b.ToUpdate
	x.xxx_hidden_ToDelete = &b.ToDelete
	return m0
}

var File_resources_wiki_activity_activity_proto protoreflect.FileDescriptor

const file_resources_wiki_activity_activity_proto_rawDesc = "" +
	"\n" +
	"&resources/wiki/activity/activity.proto\x12\x17resources.wiki.activity\x1a!codegen/dbscanner/dbscanner.proto\x1a,resources/common/content/diff_activity.proto\x1a#resources/timestamp/timestamp.proto\x1a resources/users/short/user.proto\x1a\"resources/wiki/access/access.proto\x1a\x13tagger/tagger.proto\"\xda\x04\n" +
	"\fPageActivity\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x03R\x02id\x12=\n" +
	"\n" +
	"created_at\x18\x02 \x01(\v2\x1e.resources.timestamp.TimestampR\tcreatedAt\x12\x17\n" +
	"\apage_id\x18\x03 \x01(\x03R\x06pageId\x12X\n" +
	"\ractivity_type\x18\x04 \x01(\x0e2).resources.wiki.activity.PageActivityTypeB\b\xbaH\x05\x82\x01\x02\x10\x01R\factivityType\x12+\n" +
	"\n" +
	"creator_id\x18\x05 \x01(\x05B\a\xbaH\x04\x1a\x02 \x00H\x00R\tcreatorId\x88\x01\x01\x12U\n" +
	"\acreator\x18\x06 \x01(\v2 .resources.users.short.UserShortB\x14\x9a\x84\x9e\x03\x0falias:\"creator\"H\x01R\acreator\x88\x01\x01\x12(\n" +
	"\vcreator_job\x18\a \x01(\tB\a\xbaH\x04r\x02\x18\x14R\n" +
	"creatorJob\x128\n" +
	"\x11creator_job_label\x18\b \x01(\tB\a\xbaH\x04r\x02\x182H\x02R\x0fcreatorJobLabel\x88\x01\x01\x12%\n" +
	"\x06reason\x18\t \x01(\tB\b\xbaH\x05r\x03\x18\xff\x01H\x03R\x06reason\x88\x01\x01\x12=\n" +
	"\x04data\x18\n" +
	" \x01(\v2).resources.wiki.activity.PageActivityDataR\x04dataB\r\n" +
	"\v_creator_idB\n" +
	"\n" +
	"\b_creatorB\x14\n" +
	"\x12_creator_job_labelB\t\n" +
	"\a_reason\"\xc0\x01\n" +
	"\x10PageActivityData\x12@\n" +
	"\aupdated\x18\x01 \x01(\v2$.resources.wiki.activity.PageUpdatedH\x00R\aupdated\x12S\n" +
	"\x0eaccess_updated\x18\x02 \x01(\v2*.resources.wiki.activity.PageAccessUpdatedH\x00R\raccessUpdated:\x06\xe2\xf3\x18\x02\b\x01B\r\n" +
	"\x04data\x12\x05\xbaH\x02\b\x01\"\xd0\x04\n" +
	"\vPageUpdated\x12\"\n" +
	"\n" +
	"title_diff\x18\x01 \x01(\tH\x00R\ttitleDiff\x88\x01\x01\x12K\n" +
	"\vtitle_cdiff\x18\x05 \x01(\v2%.resources.common.content.ContentDiffH\x01R\n" +
	"titleCdiff\x88\x01\x01\x12.\n" +
	"\x10description_diff\x18\x02 \x01(\tH\x02R\x0fdescriptionDiff\x88\x01\x01\x12W\n" +
	"\x11description_cdiff\x18\x06 \x01(\v2%.resources.common.content.ContentDiffH\x03R\x10descriptionCdiff\x88\x01\x01\x12&\n" +
	"\fcontent_diff\x18\x03 \x01(\tH\x04R\vcontentDiff\x88\x01\x01\x12O\n" +
	"\rcontent_cdiff\x18\a \x01(\v2%.resources.common.content.ContentDiffH\x05R\fcontentCdiff\x88\x01\x01\x12P\n" +
	"\ffiles_change\x18\x04 \x01(\v2(.resources.wiki.activity.PageFilesChangeH\x06R\vfilesChange\x88\x01\x01B\r\n" +
	"\v_title_diffB\x0e\n" +
	"\f_title_cdiffB\x13\n" +
	"\x11_description_diffB\x14\n" +
	"\x12_description_cdiffB\x0f\n" +
	"\r_content_diffB\x10\n" +
	"\x0e_content_cdiffB\x0f\n" +
	"\r_files_change\"A\n" +
	"\x0fPageFilesChange\x12\x14\n" +
	"\x05added\x18\x01 \x01(\x03R\x05added\x12\x18\n" +
	"\adeleted\x18\x02 \x01(\x03R\adeleted\"\x98\x01\n" +
	"\x11PageAccessUpdated\x12?\n" +
	"\x04jobs\x18\x01 \x01(\v2+.resources.wiki.activity.PageAccessJobsDiffR\x04jobs\x12B\n" +
	"\x05users\x18\x02 \x01(\v2,.resources.wiki.activity.PageAccessUsersDiffR\x05users\"\xfb\x01\n" +
	"\x12PageAccessJobsDiff\x12K\n" +
	"\tto_create\x18\x01 \x03(\v2$.resources.wiki.access.PageJobAccessB\b\xbaH\x05\x92\x01\x02\x10\x14R\btoCreate\x12K\n" +
	"\tto_update\x18\x02 \x03(\v2$.resources.wiki.access.PageJobAccessB\b\xbaH\x05\x92\x01\x02\x10\x14R\btoUpdate\x12K\n" +
	"\tto_delete\x18\x03 \x03(\v2$.resources.wiki.access.PageJobAccessB\b\xbaH\x05\x92\x01\x02\x10\x14R\btoDelete\"\xff\x01\n" +
	"\x13PageAccessUsersDiff\x12L\n" +
	"\tto_create\x18\x01 \x03(\v2%.resources.wiki.access.PageUserAccessB\b\xbaH\x05\x92\x01\x02\x10\x14R\btoCreate\x12L\n" +
	"\tto_update\x18\x02 \x03(\v2%.resources.wiki.access.PageUserAccessB\b\xbaH\x05\x92\x01\x02\x10\x14R\btoUpdate\x12L\n" +
	"\tto_delete\x18\x03 \x03(\v2%.resources.wiki.access.PageUserAccessB\b\xbaH\x05\x92\x01\x02\x10\x14R\btoDelete*\x89\x02\n" +
	"\x10PageActivityType\x12\"\n" +
	"\x1ePAGE_ACTIVITY_TYPE_UNSPECIFIED\x10\x00\x12\x1e\n" +
	"\x1aPAGE_ACTIVITY_TYPE_CREATED\x10\x01\x12\x1e\n" +
	"\x1aPAGE_ACTIVITY_TYPE_UPDATED\x10\x02\x12%\n" +
	"!PAGE_ACTIVITY_TYPE_ACCESS_UPDATED\x10\x03\x12$\n" +
	" PAGE_ACTIVITY_TYPE_OWNER_CHANGED\x10\x04\x12\x1e\n" +
	"\x1aPAGE_ACTIVITY_TYPE_DELETED\x10\x05\x12$\n" +
	" PAGE_ACTIVITY_TYPE_DRAFT_TOGGLED\x10\x06BXZVgithub.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/wiki/activity;wikiactivityb\x06proto3"

var file_resources_wiki_activity_activity_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_resources_wiki_activity_activity_proto_msgTypes = make([]protoimpl.MessageInfo, 7)
var file_resources_wiki_activity_activity_proto_goTypes = []any{
	(PageActivityType)(0),         // 0: resources.wiki.activity.PageActivityType
	(*PageActivity)(nil),          // 1: resources.wiki.activity.PageActivity
	(*PageActivityData)(nil),      // 2: resources.wiki.activity.PageActivityData
	(*PageUpdated)(nil),           // 3: resources.wiki.activity.PageUpdated
	(*PageFilesChange)(nil),       // 4: resources.wiki.activity.PageFilesChange
	(*PageAccessUpdated)(nil),     // 5: resources.wiki.activity.PageAccessUpdated
	(*PageAccessJobsDiff)(nil),    // 6: resources.wiki.activity.PageAccessJobsDiff
	(*PageAccessUsersDiff)(nil),   // 7: resources.wiki.activity.PageAccessUsersDiff
	(*timestamp.Timestamp)(nil),   // 8: resources.timestamp.Timestamp
	(*short.UserShort)(nil),       // 9: resources.users.short.UserShort
	(*content.ContentDiff)(nil),   // 10: resources.common.content.ContentDiff
	(*access.PageJobAccess)(nil),  // 11: resources.wiki.access.PageJobAccess
	(*access.PageUserAccess)(nil), // 12: resources.wiki.access.PageUserAccess
}
var file_resources_wiki_activity_activity_proto_depIdxs = []int32{
	8,  // 0: resources.wiki.activity.PageActivity.created_at:type_name -> resources.timestamp.Timestamp
	0,  // 1: resources.wiki.activity.PageActivity.activity_type:type_name -> resources.wiki.activity.PageActivityType
	9,  // 2: resources.wiki.activity.PageActivity.creator:type_name -> resources.users.short.UserShort
	2,  // 3: resources.wiki.activity.PageActivity.data:type_name -> resources.wiki.activity.PageActivityData
	3,  // 4: resources.wiki.activity.PageActivityData.updated:type_name -> resources.wiki.activity.PageUpdated
	5,  // 5: resources.wiki.activity.PageActivityData.access_updated:type_name -> resources.wiki.activity.PageAccessUpdated
	10, // 6: resources.wiki.activity.PageUpdated.title_cdiff:type_name -> resources.common.content.ContentDiff
	10, // 7: resources.wiki.activity.PageUpdated.description_cdiff:type_name -> resources.common.content.ContentDiff
	10, // 8: resources.wiki.activity.PageUpdated.content_cdiff:type_name -> resources.common.content.ContentDiff
	4,  // 9: resources.wiki.activity.PageUpdated.files_change:type_name -> resources.wiki.activity.PageFilesChange
	6,  // 10: resources.wiki.activity.PageAccessUpdated.jobs:type_name -> resources.wiki.activity.PageAccessJobsDiff
	7,  // 11: resources.wiki.activity.PageAccessUpdated.users:type_name -> resources.wiki.activity.PageAccessUsersDiff
	11, // 12: resources.wiki.activity.PageAccessJobsDiff.to_create:type_name -> resources.wiki.access.PageJobAccess
	11, // 13: resources.wiki.activity.PageAccessJobsDiff.to_update:type_name -> resources.wiki.access.PageJobAccess
	11, // 14: resources.wiki.activity.PageAccessJobsDiff.to_delete:type_name -> resources.wiki.access.PageJobAccess
	12, // 15: resources.wiki.activity.PageAccessUsersDiff.to_create:type_name -> resources.wiki.access.PageUserAccess
	12, // 16: resources.wiki.activity.PageAccessUsersDiff.to_update:type_name -> resources.wiki.access.PageUserAccess
	12, // 17: resources.wiki.activity.PageAccessUsersDiff.to_delete:type_name -> resources.wiki.access.PageUserAccess
	18, // [18:18] is the sub-list for method output_type
	18, // [18:18] is the sub-list for method input_type
	18, // [18:18] is the sub-list for extension type_name
	18, // [18:18] is the sub-list for extension extendee
	0,  // [0:18] is the sub-list for field type_name
}

func init() { file_resources_wiki_activity_activity_proto_init() }
func file_resources_wiki_activity_activity_proto_init() {
	if File_resources_wiki_activity_activity_proto != nil {
		return
	}
	file_resources_wiki_activity_activity_proto_msgTypes[0].OneofWrappers = []any{}
	file_resources_wiki_activity_activity_proto_msgTypes[1].OneofWrappers = []any{
		(*pageActivityData_Updated)(nil),
		(*pageActivityData_AccessUpdated)(nil),
	}
	file_resources_wiki_activity_activity_proto_msgTypes[2].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_resources_wiki_activity_activity_proto_rawDesc), len(file_resources_wiki_activity_activity_proto_rawDesc)),
			NumEnums:      1,
			NumMessages:   7,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_resources_wiki_activity_activity_proto_goTypes,
		DependencyIndexes: file_resources_wiki_activity_activity_proto_depIdxs,
		EnumInfos:         file_resources_wiki_activity_activity_proto_enumTypes,
		MessageInfos:      file_resources_wiki_activity_activity_proto_msgTypes,
	}.Build()
	File_resources_wiki_activity_activity_proto = out.File
	file_resources_wiki_activity_activity_proto_goTypes = nil
	file_resources_wiki_activity_activity_proto_depIdxs = nil
}
