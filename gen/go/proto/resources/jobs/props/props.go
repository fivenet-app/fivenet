...
func (x *JobProps) Default(job string) {
	...
	if x.GetDiscordSyncSettings() == nil {
		x.DiscordSyncSettings = &jobssettings.DiscordSyncSettings{
			DryRun:                   false,
			UserInfoSync:             false,
	...
	if x.GetDiscordSyncSettings().GetUserInfoSyncSettings() == nil {
		x.DiscordSyncSettings.UserInfoSyncSettings = &jobssettings.UserInfoSyncSettings{
			EmployeeRoleEnabled: true,
			UnemployedEnabled:   false,
			UnemployedMode:      jobssettings.UserInfoSyncUnemployedMode_USER_INFO_SYNC_UNEMPLOYED_MODE_GIVE_ROLE,
			SyncNicknames:       true,
			GroupMapping:        []*jobssettings.GroupMapping{},
		}
	}
	...
	if x.GetDiscordSyncSettings().GetStatusLogSettings() == nil {
		x.DiscordSyncSettings.StatusLogSettings = &jobssettings.StatusLogSettings{}
	}

	// Jobs Abscene Role
	if x.GetDiscordSyncSettings().GetJobsAbsenceSettings() == nil {
		x.DiscordSyncSettings.JobsAbsenceSettings = &jobssettings.JobsAbsenceSettings{
	...
	if x.GetDiscordSyncSettings().GetGroupSyncSettings() == nil {
		x.DiscordSyncSettings.GroupSyncSettings = &jobssettings.GroupSyncSettings{
	...
	if x.GetSettings() == nil {
		x.Settings = &jobssettings.JobSettings{}
	}
