...
func (x *Settings) Merge(in *Settings) *Settings {
	...
	if in.GetAccess() == nil {
		x.Access = &centrumaccess.CentrumAccess{}
	} else {
		x.Access = in.GetAccess()
	}

	if in.GetOfferedAccess() == nil {
		x.OfferedAccess = &centrumaccess.CentrumAccess{}
	} else {
...
func (x *Settings) JobHasAccess(job string, access centrumaccess.CentrumAccessLevel) bool {
	if x.GetJob() == job {
