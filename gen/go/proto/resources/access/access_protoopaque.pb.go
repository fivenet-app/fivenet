// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.10
// 	protoc        (unknown)
// source: resources/access/access.proto

//go:build protoopaque

package resourcesaccess

import (
	_ "github.com/fivenet-app/fivenet/v2026/gen/go/proto/codegen/dbscanner"
	short "github.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/users/short"
	_ "github.com/srikrsna/protoc-gen-gotag/tagger"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type Access struct {
	state                     protoimpl.MessageState  `protogen:"opaque.v1"`
	xxx_hidden_Jobs           *[]*JobAccess           `protobuf:"bytes,1,rep,name=jobs,proto3"`
	xxx_hidden_Users          *[]*UserAccess          `protobuf:"bytes,2,rep,name=users,proto3"`
	xxx_hidden_Qualifications *[]*QualificationAccess `protobuf:"bytes,3,rep,name=qualifications,proto3"`
	unknownFields             protoimpl.UnknownFields
	sizeCache                 protoimpl.SizeCache
}

func (x *Access) Reset() {
	*x = Access{}
	mi := &file_resources_access_access_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Access) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Access) ProtoMessage() {}

func (x *Access) ProtoReflect() protoreflect.Message {
	mi := &file_resources_access_access_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *Access) GetJobs() []*JobAccess {
	if x != nil {
		if x.xxx_hidden_Jobs != nil {
			return *x.xxx_hidden_Jobs
		}
	}
	return nil
}

func (x *Access) GetUsers() []*UserAccess {
	if x != nil {
		if x.xxx_hidden_Users != nil {
			return *x.xxx_hidden_Users
		}
	}
	return nil
}

func (x *Access) GetQualifications() []*QualificationAccess {
	if x != nil {
		if x.xxx_hidden_Qualifications != nil {
			return *x.xxx_hidden_Qualifications
		}
	}
	return nil
}

func (x *Access) SetJobs(v []*JobAccess) {
	x.xxx_hidden_Jobs = &v
}

func (x *Access) SetUsers(v []*UserAccess) {
	x.xxx_hidden_Users = &v
}

func (x *Access) SetQualifications(v []*QualificationAccess) {
	x.xxx_hidden_Qualifications = &v
}

type Access_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Jobs           []*JobAccess
	Users          []*UserAccess
	Qualifications []*QualificationAccess
}

func (b0 Access_builder) Build() *Access {
	m0 := &Access{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Jobs = &b.Jobs
	x.xxx_hidden_Users = &b.Users
	x.xxx_hidden_Qualifications = &b.Qualifications
	return m0
}

type JobAccess struct {
	state                     protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Id             int64                  `protobuf:"varint,1,opt,name=id,proto3"`
	xxx_hidden_TargetId       int64                  `protobuf:"varint,3,opt,name=target_id,json=targetId,proto3"`
	xxx_hidden_Job            string                 `protobuf:"bytes,4,opt,name=job,proto3"`
	xxx_hidden_JobLabel       *string                `protobuf:"bytes,5,opt,name=job_label,json=jobLabel,proto3,oneof"`
	xxx_hidden_MinimumGrade   int32                  `protobuf:"varint,6,opt,name=minimum_grade,json=minimumGrade,proto3"`
	xxx_hidden_JobGradeLabel  *string                `protobuf:"bytes,7,opt,name=job_grade_label,json=jobGradeLabel,proto3,oneof"`
	xxx_hidden_Access         int32                  `protobuf:"varint,8,opt,name=access,proto3"`
	xxx_hidden_Required       bool                   `protobuf:"varint,9,opt,name=required,proto3,oneof"`
	xxx_hidden_RequiredAccess int32                  `protobuf:"varint,10,opt,name=required_access,json=requiredAccess,proto3,oneof"`
	XXX_raceDetectHookData    protoimpl.RaceDetectHookData
	XXX_presence              [1]uint32
	unknownFields             protoimpl.UnknownFields
	sizeCache                 protoimpl.SizeCache
}

func (x *JobAccess) Reset() {
	*x = JobAccess{}
	mi := &file_resources_access_access_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *JobAccess) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*JobAccess) ProtoMessage() {}

func (x *JobAccess) ProtoReflect() protoreflect.Message {
	mi := &file_resources_access_access_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *JobAccess) GetId() int64 {
	if x != nil {
		return x.xxx_hidden_Id
	}
	return 0
}

func (x *JobAccess) GetTargetId() int64 {
	if x != nil {
		return x.xxx_hidden_TargetId
	}
	return 0
}

func (x *JobAccess) GetJob() string {
	if x != nil {
		return x.xxx_hidden_Job
	}
	return ""
}

func (x *JobAccess) GetJobLabel() string {
	if x != nil {
		if x.xxx_hidden_JobLabel != nil {
			return *x.xxx_hidden_JobLabel
		}
		return ""
	}
	return ""
}

func (x *JobAccess) GetMinimumGrade() int32 {
	if x != nil {
		return x.xxx_hidden_MinimumGrade
	}
	return 0
}

func (x *JobAccess) GetJobGradeLabel() string {
	if x != nil {
		if x.xxx_hidden_JobGradeLabel != nil {
			return *x.xxx_hidden_JobGradeLabel
		}
		return ""
	}
	return ""
}

func (x *JobAccess) GetAccess() int32 {
	if x != nil {
		return x.xxx_hidden_Access
	}
	return 0
}

func (x *JobAccess) GetRequired() bool {
	if x != nil {
		return x.xxx_hidden_Required
	}
	return false
}

func (x *JobAccess) GetRequiredAccess() int32 {
	if x != nil {
		return x.xxx_hidden_RequiredAccess
	}
	return 0
}

func (x *JobAccess) SetId(v int64) {
	x.xxx_hidden_Id = v
}

func (x *JobAccess) SetTargetId(v int64) {
	x.xxx_hidden_TargetId = v
}

func (x *JobAccess) SetJob(v string) {
	x.xxx_hidden_Job = v
}

func (x *JobAccess) SetJobLabel(v string) {
	x.xxx_hidden_JobLabel = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 3, 9)
}

func (x *JobAccess) SetMinimumGrade(v int32) {
	x.xxx_hidden_MinimumGrade = v
}

func (x *JobAccess) SetJobGradeLabel(v string) {
	x.xxx_hidden_JobGradeLabel = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 5, 9)
}

func (x *JobAccess) SetAccess(v int32) {
	x.xxx_hidden_Access = v
}

func (x *JobAccess) SetRequired(v bool) {
	x.xxx_hidden_Required = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 7, 9)
}

func (x *JobAccess) SetRequiredAccess(v int32) {
	x.xxx_hidden_RequiredAccess = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 8, 9)
}

func (x *JobAccess) HasJobLabel() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 3)
}

func (x *JobAccess) HasJobGradeLabel() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 5)
}

func (x *JobAccess) HasRequired() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 7)
}

func (x *JobAccess) HasRequiredAccess() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 8)
}

func (x *JobAccess) ClearJobLabel() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 3)
	x.xxx_hidden_JobLabel = nil
}

func (x *JobAccess) ClearJobGradeLabel() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 5)
	x.xxx_hidden_JobGradeLabel = nil
}

func (x *JobAccess) ClearRequired() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 7)
	x.xxx_hidden_Required = false
}

func (x *JobAccess) ClearRequiredAccess() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 8)
	x.xxx_hidden_RequiredAccess = 0
}

type JobAccess_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Id             int64
	TargetId       int64
	Job            string
	JobLabel       *string
	MinimumGrade   int32
	JobGradeLabel  *string
	Access         int32
	Required       *bool
	RequiredAccess *int32
}

func (b0 JobAccess_builder) Build() *JobAccess {
	m0 := &JobAccess{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Id = b.Id
	x.xxx_hidden_TargetId = b.TargetId
	x.xxx_hidden_Job = b.Job
	if b.JobLabel != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 3, 9)
		x.xxx_hidden_JobLabel = b.JobLabel
	}
	x.xxx_hidden_MinimumGrade = b.MinimumGrade
	if b.JobGradeLabel != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 5, 9)
		x.xxx_hidden_JobGradeLabel = b.JobGradeLabel
	}
	x.xxx_hidden_Access = b.Access
	if b.Required != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 7, 9)
		x.xxx_hidden_Required = *b.Required
	}
	if b.RequiredAccess != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 8, 9)
		x.xxx_hidden_RequiredAccess = *b.RequiredAccess
	}
	return m0
}

type UserAccess struct {
	state                     protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Id             int64                  `protobuf:"varint,1,opt,name=id,proto3"`
	xxx_hidden_TargetId       int64                  `protobuf:"varint,3,opt,name=target_id,json=targetId,proto3"`
	xxx_hidden_UserId         int32                  `protobuf:"varint,4,opt,name=user_id,json=userId,proto3"`
	xxx_hidden_User           *short.UserShort       `protobuf:"bytes,5,opt,name=user,proto3,oneof"`
	xxx_hidden_Access         int32                  `protobuf:"varint,6,opt,name=access,proto3"`
	xxx_hidden_Required       bool                   `protobuf:"varint,7,opt,name=required,proto3,oneof"`
	xxx_hidden_RequiredAccess int32                  `protobuf:"varint,8,opt,name=required_access,json=requiredAccess,proto3,oneof"`
	XXX_raceDetectHookData    protoimpl.RaceDetectHookData
	XXX_presence              [1]uint32
	unknownFields             protoimpl.UnknownFields
	sizeCache                 protoimpl.SizeCache
}

func (x *UserAccess) Reset() {
	*x = UserAccess{}
	mi := &file_resources_access_access_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UserAccess) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserAccess) ProtoMessage() {}

func (x *UserAccess) ProtoReflect() protoreflect.Message {
	mi := &file_resources_access_access_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *UserAccess) GetId() int64 {
	if x != nil {
		return x.xxx_hidden_Id
	}
	return 0
}

func (x *UserAccess) GetTargetId() int64 {
	if x != nil {
		return x.xxx_hidden_TargetId
	}
	return 0
}

func (x *UserAccess) GetUserId() int32 {
	if x != nil {
		return x.xxx_hidden_UserId
	}
	return 0
}

func (x *UserAccess) GetUser() *short.UserShort {
	if x != nil {
		return x.xxx_hidden_User
	}
	return nil
}

func (x *UserAccess) GetAccess() int32 {
	if x != nil {
		return x.xxx_hidden_Access
	}
	return 0
}

func (x *UserAccess) GetRequired() bool {
	if x != nil {
		return x.xxx_hidden_Required
	}
	return false
}

func (x *UserAccess) GetRequiredAccess() int32 {
	if x != nil {
		return x.xxx_hidden_RequiredAccess
	}
	return 0
}

func (x *UserAccess) SetId(v int64) {
	x.xxx_hidden_Id = v
}

func (x *UserAccess) SetTargetId(v int64) {
	x.xxx_hidden_TargetId = v
}

func (x *UserAccess) SetUserId(v int32) {
	x.xxx_hidden_UserId = v
}

func (x *UserAccess) SetUser(v *short.UserShort) {
	x.xxx_hidden_User = v
}

func (x *UserAccess) SetAccess(v int32) {
	x.xxx_hidden_Access = v
}

func (x *UserAccess) SetRequired(v bool) {
	x.xxx_hidden_Required = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 5, 7)
}

func (x *UserAccess) SetRequiredAccess(v int32) {
	x.xxx_hidden_RequiredAccess = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 6, 7)
}

func (x *UserAccess) HasUser() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_User != nil
}

func (x *UserAccess) HasRequired() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 5)
}

func (x *UserAccess) HasRequiredAccess() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 6)
}

func (x *UserAccess) ClearUser() {
	x.xxx_hidden_User = nil
}

func (x *UserAccess) ClearRequired() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 5)
	x.xxx_hidden_Required = false
}

func (x *UserAccess) ClearRequiredAccess() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 6)
	x.xxx_hidden_RequiredAccess = 0
}

type UserAccess_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Id             int64
	TargetId       int64
	UserId         int32
	User           *short.UserShort
	Access         int32
	Required       *bool
	RequiredAccess *int32
}

func (b0 UserAccess_builder) Build() *UserAccess {
	m0 := &UserAccess{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Id = b.Id
	x.xxx_hidden_TargetId = b.TargetId
	x.xxx_hidden_UserId = b.UserId
	x.xxx_hidden_User = b.User
	x.xxx_hidden_Access = b.Access
	if b.Required != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 5, 7)
		x.xxx_hidden_Required = *b.Required
	}
	if b.RequiredAccess != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 6, 7)
		x.xxx_hidden_RequiredAccess = *b.RequiredAccess
	}
	return m0
}

type QualificationAccess struct {
	state                      protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Id              int64                  `protobuf:"varint,1,opt,name=id,proto3"`
	xxx_hidden_TargetId        int64                  `protobuf:"varint,3,opt,name=target_id,json=targetId,proto3"`
	xxx_hidden_QualificationId int64                  `protobuf:"varint,4,opt,name=qualification_id,json=qualificationId,proto3"`
	xxx_hidden_Access          int32                  `protobuf:"varint,6,opt,name=access,proto3"`
	xxx_hidden_Required        bool                   `protobuf:"varint,7,opt,name=required,proto3,oneof"`
	xxx_hidden_RequiredAccess  int32                  `protobuf:"varint,8,opt,name=required_access,json=requiredAccess,proto3,oneof"`
	XXX_raceDetectHookData     protoimpl.RaceDetectHookData
	XXX_presence               [1]uint32
	unknownFields              protoimpl.UnknownFields
	sizeCache                  protoimpl.SizeCache
}

func (x *QualificationAccess) Reset() {
	*x = QualificationAccess{}
	mi := &file_resources_access_access_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QualificationAccess) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QualificationAccess) ProtoMessage() {}

func (x *QualificationAccess) ProtoReflect() protoreflect.Message {
	mi := &file_resources_access_access_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *QualificationAccess) GetId() int64 {
	if x != nil {
		return x.xxx_hidden_Id
	}
	return 0
}

func (x *QualificationAccess) GetTargetId() int64 {
	if x != nil {
		return x.xxx_hidden_TargetId
	}
	return 0
}

func (x *QualificationAccess) GetQualificationId() int64 {
	if x != nil {
		return x.xxx_hidden_QualificationId
	}
	return 0
}

func (x *QualificationAccess) GetAccess() int32 {
	if x != nil {
		return x.xxx_hidden_Access
	}
	return 0
}

func (x *QualificationAccess) GetRequired() bool {
	if x != nil {
		return x.xxx_hidden_Required
	}
	return false
}

func (x *QualificationAccess) GetRequiredAccess() int32 {
	if x != nil {
		return x.xxx_hidden_RequiredAccess
	}
	return 0
}

func (x *QualificationAccess) SetId(v int64) {
	x.xxx_hidden_Id = v
}

func (x *QualificationAccess) SetTargetId(v int64) {
	x.xxx_hidden_TargetId = v
}

func (x *QualificationAccess) SetQualificationId(v int64) {
	x.xxx_hidden_QualificationId = v
}

func (x *QualificationAccess) SetAccess(v int32) {
	x.xxx_hidden_Access = v
}

func (x *QualificationAccess) SetRequired(v bool) {
	x.xxx_hidden_Required = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 4, 6)
}

func (x *QualificationAccess) SetRequiredAccess(v int32) {
	x.xxx_hidden_RequiredAccess = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 5, 6)
}

func (x *QualificationAccess) HasRequired() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 4)
}

func (x *QualificationAccess) HasRequiredAccess() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 5)
}

func (x *QualificationAccess) ClearRequired() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 4)
	x.xxx_hidden_Required = false
}

func (x *QualificationAccess) ClearRequiredAccess() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 5)
	x.xxx_hidden_RequiredAccess = 0
}

type QualificationAccess_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Id              int64
	TargetId        int64
	QualificationId int64
	Access          int32
	Required        *bool
	RequiredAccess  *int32
}

func (b0 QualificationAccess_builder) Build() *QualificationAccess {
	m0 := &QualificationAccess{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Id = b.Id
	x.xxx_hidden_TargetId = b.TargetId
	x.xxx_hidden_QualificationId = b.QualificationId
	x.xxx_hidden_Access = b.Access
	if b.Required != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 4, 6)
		x.xxx_hidden_Required = *b.Required
	}
	if b.RequiredAccess != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 5, 6)
		x.xxx_hidden_RequiredAccess = *b.RequiredAccess
	}
	return m0
}

var File_resources_access_access_proto protoreflect.FileDescriptor

const file_resources_access_access_proto_rawDesc = "" +
	"\n" +
	"\x1dresources/access/access.proto\x12\x10resources.access\x1a!codegen/dbscanner/dbscanner.proto\x1a resources/users/short/user.proto\x1a\x13tagger/tagger.proto\"\xb2\x02\n" +
	"\x06Access\x12P\n" +
	"\x04jobs\x18\x01 \x03(\v2\x1b.resources.access.JobAccessB\x1f\x9a\x84\x9e\x03\x12alias:\"job_access\"\xbaH\x05\x92\x01\x02\x10\x14R\x04jobs\x12T\n" +
	"\x05users\x18\x02 \x03(\v2\x1c.resources.access.UserAccessB \x9a\x84\x9e\x03\x13alias:\"user_access\"\xbaH\x05\x92\x01\x02\x10\x14R\x05users\x12x\n" +
	"\x0equalifications\x18\x03 \x03(\v2%.resources.access.QualificationAccessB)\x9a\x84\x9e\x03\x1calias:\"qualification_access\"\xbaH\x05\x92\x01\x02\x10\x14R\x0equalifications:\x06\xe2\xf3\x18\x02\b\x01\"\x92\x03\n" +
	"\tJobAccess\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x03R\x02id\x12\x1b\n" +
	"\ttarget_id\x18\x03 \x01(\x03R\btargetId\x12\x19\n" +
	"\x03job\x18\x04 \x01(\tB\a\xbaH\x04r\x02\x18\x14R\x03job\x12)\n" +
	"\tjob_label\x18\x05 \x01(\tB\a\xbaH\x04r\x02\x182H\x00R\bjobLabel\x88\x01\x01\x12,\n" +
	"\rminimum_grade\x18\x06 \x01(\x05B\a\xbaH\x04\x1a\x02(\x00R\fminimumGrade\x124\n" +
	"\x0fjob_grade_label\x18\a \x01(\tB\a\xbaH\x04r\x02\x182H\x01R\rjobGradeLabel\x88\x01\x01\x12\x16\n" +
	"\x06access\x18\b \x01(\x05R\x06access\x12\x1f\n" +
	"\brequired\x18\t \x01(\bH\x02R\brequired\x88\x01\x01\x12,\n" +
	"\x0frequired_access\x18\n" +
	" \x01(\x05H\x03R\x0erequiredAccess\x88\x01\x01B\f\n" +
	"\n" +
	"_job_labelB\x12\n" +
	"\x10_job_grade_labelB\v\n" +
	"\t_requiredB\x12\n" +
	"\x10_required_accessJ\x04\b\x02\x10\x03\"\xad\x02\n" +
	"\n" +
	"UserAccess\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x03R\x02id\x12\x1b\n" +
	"\ttarget_id\x18\x03 \x01(\x03R\btargetId\x12 \n" +
	"\auser_id\x18\x04 \x01(\x05B\a\xbaH\x04\x1a\x02 \x00R\x06userId\x129\n" +
	"\x04user\x18\x05 \x01(\v2 .resources.users.short.UserShortH\x00R\x04user\x88\x01\x01\x12\x16\n" +
	"\x06access\x18\x06 \x01(\x05R\x06access\x12\x1f\n" +
	"\brequired\x18\a \x01(\bH\x01R\brequired\x88\x01\x01\x12,\n" +
	"\x0frequired_access\x18\b \x01(\x05H\x02R\x0erequiredAccess\x88\x01\x01B\a\n" +
	"\x05_userB\v\n" +
	"\t_requiredB\x12\n" +
	"\x10_required_accessJ\x04\b\x02\x10\x03\"\x84\x02\n" +
	"\x13QualificationAccess\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x03R\x02id\x12\x1b\n" +
	"\ttarget_id\x18\x03 \x01(\x03R\btargetId\x122\n" +
	"\x10qualification_id\x18\x04 \x01(\x03B\a\xbaH\x04\"\x02 \x00R\x0fqualificationId\x12\x16\n" +
	"\x06access\x18\x06 \x01(\x05R\x06access\x12\x1f\n" +
	"\brequired\x18\a \x01(\bH\x00R\brequired\x88\x01\x01\x12,\n" +
	"\x0frequired_access\x18\b \x01(\x05H\x01R\x0erequiredAccess\x88\x01\x01B\v\n" +
	"\t_requiredB\x12\n" +
	"\x10_required_accessJ\x04\b\x02\x10\x03BTZRgithub.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/access;resourcesaccessb\x06proto3"

var file_resources_access_access_proto_msgTypes = make([]protoimpl.MessageInfo, 4)
var file_resources_access_access_proto_goTypes = []any{
	(*Access)(nil),              // 0: resources.access.Access
	(*JobAccess)(nil),           // 1: resources.access.JobAccess
	(*UserAccess)(nil),          // 2: resources.access.UserAccess
	(*QualificationAccess)(nil), // 3: resources.access.QualificationAccess
	(*short.UserShort)(nil),     // 4: resources.users.short.UserShort
}
var file_resources_access_access_proto_depIdxs = []int32{
	1, // 0: resources.access.Access.jobs:type_name -> resources.access.JobAccess
	2, // 1: resources.access.Access.users:type_name -> resources.access.UserAccess
	3, // 2: resources.access.Access.qualifications:type_name -> resources.access.QualificationAccess
	4, // 3: resources.access.UserAccess.user:type_name -> resources.users.short.UserShort
	4, // [4:4] is the sub-list for method output_type
	4, // [4:4] is the sub-list for method input_type
	4, // [4:4] is the sub-list for extension type_name
	4, // [4:4] is the sub-list for extension extendee
	0, // [0:4] is the sub-list for field type_name
}

func init() { file_resources_access_access_proto_init() }
func file_resources_access_access_proto_init() {
	if File_resources_access_access_proto != nil {
		return
	}
	file_resources_access_access_proto_msgTypes[1].OneofWrappers = []any{}
	file_resources_access_access_proto_msgTypes[2].OneofWrappers = []any{}
	file_resources_access_access_proto_msgTypes[3].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_resources_access_access_proto_rawDesc), len(file_resources_access_access_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   4,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_resources_access_access_proto_goTypes,
		DependencyIndexes: file_resources_access_access_proto_depIdxs,
		MessageInfos:      file_resources_access_access_proto_msgTypes,
	}.Build()
	File_resources_access_access_proto = out.File
	file_resources_access_access_proto_goTypes = nil
	file_resources_access_access_proto_depIdxs = nil
}
