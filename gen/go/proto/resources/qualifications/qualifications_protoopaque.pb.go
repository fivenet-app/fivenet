// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.10
// 	protoc        (unknown)
// source: resources/qualifications/qualifications.proto

//go:build protoopaque

package qualifications

import (
	_ "github.com/fivenet-app/fivenet/v2026/gen/go/proto/codegen/dbscanner"
	_ "github.com/fivenet-app/fivenet/v2026/gen/go/proto/codegen/sanitizer"
	content "github.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/common/content"
	file "github.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/file"
	access "github.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/qualifications/access"
	exam "github.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/qualifications/exam"
	timestamp "github.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/timestamp"
	short "github.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/users/short"
	_ "github.com/srikrsna/protoc-gen-gotag/tagger"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type RequestStatus int32

const (
	RequestStatus_REQUEST_STATUS_UNSPECIFIED  RequestStatus = 0
	RequestStatus_REQUEST_STATUS_PENDING      RequestStatus = 1
	RequestStatus_REQUEST_STATUS_DENIED       RequestStatus = 2
	RequestStatus_REQUEST_STATUS_ACCEPTED     RequestStatus = 3
	RequestStatus_REQUEST_STATUS_EXAM_STARTED RequestStatus = 4
	RequestStatus_REQUEST_STATUS_EXAM_GRADING RequestStatus = 5
	RequestStatus_REQUEST_STATUS_COMPLETED    RequestStatus = 6
)

// Enum value maps for RequestStatus.
var (
	RequestStatus_name = map[int32]string{
		0: "REQUEST_STATUS_UNSPECIFIED",
		1: "REQUEST_STATUS_PENDING",
		2: "REQUEST_STATUS_DENIED",
		3: "REQUEST_STATUS_ACCEPTED",
		4: "REQUEST_STATUS_EXAM_STARTED",
		5: "REQUEST_STATUS_EXAM_GRADING",
		6: "REQUEST_STATUS_COMPLETED",
	}
	RequestStatus_value = map[string]int32{
		"REQUEST_STATUS_UNSPECIFIED":  0,
		"REQUEST_STATUS_PENDING":      1,
		"REQUEST_STATUS_DENIED":       2,
		"REQUEST_STATUS_ACCEPTED":     3,
		"REQUEST_STATUS_EXAM_STARTED": 4,
		"REQUEST_STATUS_EXAM_GRADING": 5,
		"REQUEST_STATUS_COMPLETED":    6,
	}
)

func (x RequestStatus) Enum() *RequestStatus {
	p := new(RequestStatus)
	*p = x
	return p
}

func (x RequestStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (RequestStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_resources_qualifications_qualifications_proto_enumTypes[0].Descriptor()
}

func (RequestStatus) Type() protoreflect.EnumType {
	return &file_resources_qualifications_qualifications_proto_enumTypes[0]
}

func (x RequestStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

type ResultStatus int32

const (
	ResultStatus_RESULT_STATUS_UNSPECIFIED ResultStatus = 0
	ResultStatus_RESULT_STATUS_PENDING     ResultStatus = 1
	ResultStatus_RESULT_STATUS_FAILED      ResultStatus = 2
	ResultStatus_RESULT_STATUS_SUCCESSFUL  ResultStatus = 3
)

// Enum value maps for ResultStatus.
var (
	ResultStatus_name = map[int32]string{
		0: "RESULT_STATUS_UNSPECIFIED",
		1: "RESULT_STATUS_PENDING",
		2: "RESULT_STATUS_FAILED",
		3: "RESULT_STATUS_SUCCESSFUL",
	}
	ResultStatus_value = map[string]int32{
		"RESULT_STATUS_UNSPECIFIED": 0,
		"RESULT_STATUS_PENDING":     1,
		"RESULT_STATUS_FAILED":      2,
		"RESULT_STATUS_SUCCESSFUL":  3,
	}
)

func (x ResultStatus) Enum() *ResultStatus {
	p := new(ResultStatus)
	*p = x
	return p
}

func (x ResultStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ResultStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_resources_qualifications_qualifications_proto_enumTypes[1].Descriptor()
}

func (ResultStatus) Type() protoreflect.EnumType {
	return &file_resources_qualifications_qualifications_proto_enumTypes[1]
}

func (x ResultStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

type Qualification struct {
	state                         protoimpl.MessageState          `protogen:"opaque.v1"`
	xxx_hidden_Id                 int64                           `protobuf:"varint,1,opt,name=id,proto3"`
	xxx_hidden_CreatedAt          *timestamp.Timestamp            `protobuf:"bytes,2,opt,name=created_at,json=createdAt,proto3,oneof"`
	xxx_hidden_UpdatedAt          *timestamp.Timestamp            `protobuf:"bytes,3,opt,name=updated_at,json=updatedAt,proto3,oneof"`
	xxx_hidden_DeletedAt          *timestamp.Timestamp            `protobuf:"bytes,4,opt,name=deleted_at,json=deletedAt,proto3,oneof"`
	xxx_hidden_Job                string                          `protobuf:"bytes,5,opt,name=job,proto3"`
	xxx_hidden_Weight             uint32                          `protobuf:"varint,6,opt,name=weight,proto3"`
	xxx_hidden_Closed             bool                            `protobuf:"varint,7,opt,name=closed,proto3"`
	xxx_hidden_Draft              bool                            `protobuf:"varint,8,opt,name=draft,proto3"`
	xxx_hidden_Public             bool                            `protobuf:"varint,9,opt,name=public,proto3"`
	xxx_hidden_Abbreviation       string                          `protobuf:"bytes,10,opt,name=abbreviation,proto3"`
	xxx_hidden_Title              string                          `protobuf:"bytes,11,opt,name=title,proto3"`
	xxx_hidden_Description        *string                         `protobuf:"bytes,12,opt,name=description,proto3,oneof"`
	xxx_hidden_Content            *content.Content                `protobuf:"bytes,13,opt,name=content,proto3"`
	xxx_hidden_CreatorId          int32                           `protobuf:"varint,14,opt,name=creator_id,json=creatorId,proto3,oneof"`
	xxx_hidden_Creator            *short.UserShort                `protobuf:"bytes,15,opt,name=creator,proto3,oneof"`
	xxx_hidden_CreatorJob         string                          `protobuf:"bytes,16,opt,name=creator_job,json=creatorJob,proto3"`
	xxx_hidden_Access             *access.QualificationAccess     `protobuf:"bytes,17,opt,name=access,proto3"`
	xxx_hidden_Requirements       *[]*QualificationRequirement    `protobuf:"bytes,18,rep,name=requirements,proto3"`
	xxx_hidden_DiscordSyncEnabled bool                            `protobuf:"varint,19,opt,name=discord_sync_enabled,json=discordSyncEnabled,proto3"`
	xxx_hidden_DiscordSettings    *QualificationDiscordSettings   `protobuf:"bytes,20,opt,name=discord_settings,json=discordSettings,proto3,oneof"`
	xxx_hidden_ExamMode           exam.QualificationExamMode      `protobuf:"varint,21,opt,name=exam_mode,json=examMode,proto3,enum=resources.qualifications.exam.QualificationExamMode"`
	xxx_hidden_ExamSettings       *exam.QualificationExamSettings `protobuf:"bytes,22,opt,name=exam_settings,json=examSettings,proto3,oneof"`
	xxx_hidden_Exam               *exam.ExamQuestions             `protobuf:"bytes,23,opt,name=exam,proto3,oneof"`
	xxx_hidden_Result             *QualificationResult            `protobuf:"bytes,24,opt,name=result,proto3,oneof"`
	xxx_hidden_Request            *QualificationRequest           `protobuf:"bytes,25,opt,name=request,proto3,oneof"`
	xxx_hidden_LabelSyncEnabled   bool                            `protobuf:"varint,26,opt,name=label_sync_enabled,json=labelSyncEnabled,proto3"`
	xxx_hidden_LabelSyncFormat    *string                         `protobuf:"bytes,27,opt,name=label_sync_format,json=labelSyncFormat,proto3,oneof"`
	xxx_hidden_Files              *[]*file.File                   `protobuf:"bytes,28,rep,name=files,proto3"`
	XXX_raceDetectHookData        protoimpl.RaceDetectHookData
	XXX_presence                  [1]uint32
	unknownFields                 protoimpl.UnknownFields
	sizeCache                     protoimpl.SizeCache
}

func (x *Qualification) Reset() {
	*x = Qualification{}
	mi := &file_resources_qualifications_qualifications_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Qualification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Qualification) ProtoMessage() {}

func (x *Qualification) ProtoReflect() protoreflect.Message {
	mi := &file_resources_qualifications_qualifications_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *Qualification) GetId() int64 {
	if x != nil {
		return x.xxx_hidden_Id
	}
	return 0
}

func (x *Qualification) GetCreatedAt() *timestamp.Timestamp {
	if x != nil {
		return x.xxx_hidden_CreatedAt
	}
	return nil
}

func (x *Qualification) GetUpdatedAt() *timestamp.Timestamp {
	if x != nil {
		return x.xxx_hidden_UpdatedAt
	}
	return nil
}

func (x *Qualification) GetDeletedAt() *timestamp.Timestamp {
	if x != nil {
		return x.xxx_hidden_DeletedAt
	}
	return nil
}

func (x *Qualification) GetJob() string {
	if x != nil {
		return x.xxx_hidden_Job
	}
	return ""
}

func (x *Qualification) GetWeight() uint32 {
	if x != nil {
		return x.xxx_hidden_Weight
	}
	return 0
}

func (x *Qualification) GetClosed() bool {
	if x != nil {
		return x.xxx_hidden_Closed
	}
	return false
}

func (x *Qualification) GetDraft() bool {
	if x != nil {
		return x.xxx_hidden_Draft
	}
	return false
}

func (x *Qualification) GetPublic() bool {
	if x != nil {
		return x.xxx_hidden_Public
	}
	return false
}

func (x *Qualification) GetAbbreviation() string {
	if x != nil {
		return x.xxx_hidden_Abbreviation
	}
	return ""
}

func (x *Qualification) GetTitle() string {
	if x != nil {
		return x.xxx_hidden_Title
	}
	return ""
}

func (x *Qualification) GetDescription() string {
	if x != nil {
		if x.xxx_hidden_Description != nil {
			return *x.xxx_hidden_Description
		}
		return ""
	}
	return ""
}

func (x *Qualification) GetContent() *content.Content {
	if x != nil {
		return x.xxx_hidden_Content
	}
	return nil
}

func (x *Qualification) GetCreatorId() int32 {
	if x != nil {
		return x.xxx_hidden_CreatorId
	}
	return 0
}

func (x *Qualification) GetCreator() *short.UserShort {
	if x != nil {
		return x.xxx_hidden_Creator
	}
	return nil
}

func (x *Qualification) GetCreatorJob() string {
	if x != nil {
		return x.xxx_hidden_CreatorJob
	}
	return ""
}

func (x *Qualification) GetAccess() *access.QualificationAccess {
	if x != nil {
		return x.xxx_hidden_Access
	}
	return nil
}

func (x *Qualification) GetRequirements() []*QualificationRequirement {
	if x != nil {
		if x.xxx_hidden_Requirements != nil {
			return *x.xxx_hidden_Requirements
		}
	}
	return nil
}

func (x *Qualification) GetDiscordSyncEnabled() bool {
	if x != nil {
		return x.xxx_hidden_DiscordSyncEnabled
	}
	return false
}

func (x *Qualification) GetDiscordSettings() *QualificationDiscordSettings {
	if x != nil {
		return x.xxx_hidden_DiscordSettings
	}
	return nil
}

func (x *Qualification) GetExamMode() exam.QualificationExamMode {
	if x != nil {
		return x.xxx_hidden_ExamMode
	}
	return exam.QualificationExamMode(0)
}

func (x *Qualification) GetExamSettings() *exam.QualificationExamSettings {
	if x != nil {
		return x.xxx_hidden_ExamSettings
	}
	return nil
}

func (x *Qualification) GetExam() *exam.ExamQuestions {
	if x != nil {
		return x.xxx_hidden_Exam
	}
	return nil
}

func (x *Qualification) GetResult() *QualificationResult {
	if x != nil {
		return x.xxx_hidden_Result
	}
	return nil
}

func (x *Qualification) GetRequest() *QualificationRequest {
	if x != nil {
		return x.xxx_hidden_Request
	}
	return nil
}

func (x *Qualification) GetLabelSyncEnabled() bool {
	if x != nil {
		return x.xxx_hidden_LabelSyncEnabled
	}
	return false
}

func (x *Qualification) GetLabelSyncFormat() string {
	if x != nil {
		if x.xxx_hidden_LabelSyncFormat != nil {
			return *x.xxx_hidden_LabelSyncFormat
		}
		return ""
	}
	return ""
}

func (x *Qualification) GetFiles() []*file.File {
	if x != nil {
		if x.xxx_hidden_Files != nil {
			return *x.xxx_hidden_Files
		}
	}
	return nil
}

func (x *Qualification) SetId(v int64) {
	x.xxx_hidden_Id = v
}

func (x *Qualification) SetCreatedAt(v *timestamp.Timestamp) {
	x.xxx_hidden_CreatedAt = v
}

func (x *Qualification) SetUpdatedAt(v *timestamp.Timestamp) {
	x.xxx_hidden_UpdatedAt = v
}

func (x *Qualification) SetDeletedAt(v *timestamp.Timestamp) {
	x.xxx_hidden_DeletedAt = v
}

func (x *Qualification) SetJob(v string) {
	x.xxx_hidden_Job = v
}

func (x *Qualification) SetWeight(v uint32) {
	x.xxx_hidden_Weight = v
}

func (x *Qualification) SetClosed(v bool) {
	x.xxx_hidden_Closed = v
}

func (x *Qualification) SetDraft(v bool) {
	x.xxx_hidden_Draft = v
}

func (x *Qualification) SetPublic(v bool) {
	x.xxx_hidden_Public = v
}

func (x *Qualification) SetAbbreviation(v string) {
	x.xxx_hidden_Abbreviation = v
}

func (x *Qualification) SetTitle(v string) {
	x.xxx_hidden_Title = v
}

func (x *Qualification) SetDescription(v string) {
	x.xxx_hidden_Description = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 11, 28)
}

func (x *Qualification) SetContent(v *content.Content) {
	x.xxx_hidden_Content = v
}

func (x *Qualification) SetCreatorId(v int32) {
	x.xxx_hidden_CreatorId = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 13, 28)
}

func (x *Qualification) SetCreator(v *short.UserShort) {
	x.xxx_hidden_Creator = v
}

func (x *Qualification) SetCreatorJob(v string) {
	x.xxx_hidden_CreatorJob = v
}

func (x *Qualification) SetAccess(v *access.QualificationAccess) {
	x.xxx_hidden_Access = v
}

func (x *Qualification) SetRequirements(v []*QualificationRequirement) {
	x.xxx_hidden_Requirements = &v
}

func (x *Qualification) SetDiscordSyncEnabled(v bool) {
	x.xxx_hidden_DiscordSyncEnabled = v
}

func (x *Qualification) SetDiscordSettings(v *QualificationDiscordSettings) {
	x.xxx_hidden_DiscordSettings = v
}

func (x *Qualification) SetExamMode(v exam.QualificationExamMode) {
	x.xxx_hidden_ExamMode = v
}

func (x *Qualification) SetExamSettings(v *exam.QualificationExamSettings) {
	x.xxx_hidden_ExamSettings = v
}

func (x *Qualification) SetExam(v *exam.ExamQuestions) {
	x.xxx_hidden_Exam = v
}

func (x *Qualification) SetResult(v *QualificationResult) {
	x.xxx_hidden_Result = v
}

func (x *Qualification) SetRequest(v *QualificationRequest) {
	x.xxx_hidden_Request = v
}

func (x *Qualification) SetLabelSyncEnabled(v bool) {
	x.xxx_hidden_LabelSyncEnabled = v
}

func (x *Qualification) SetLabelSyncFormat(v string) {
	x.xxx_hidden_LabelSyncFormat = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 26, 28)
}

func (x *Qualification) SetFiles(v []*file.File) {
	x.xxx_hidden_Files = &v
}

func (x *Qualification) HasCreatedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_CreatedAt != nil
}

func (x *Qualification) HasUpdatedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_UpdatedAt != nil
}

func (x *Qualification) HasDeletedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_DeletedAt != nil
}

func (x *Qualification) HasDescription() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 11)
}

func (x *Qualification) HasContent() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Content != nil
}

func (x *Qualification) HasCreatorId() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 13)
}

func (x *Qualification) HasCreator() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Creator != nil
}

func (x *Qualification) HasAccess() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Access != nil
}

func (x *Qualification) HasDiscordSettings() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_DiscordSettings != nil
}

func (x *Qualification) HasExamSettings() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_ExamSettings != nil
}

func (x *Qualification) HasExam() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Exam != nil
}

func (x *Qualification) HasResult() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Result != nil
}

func (x *Qualification) HasRequest() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Request != nil
}

func (x *Qualification) HasLabelSyncFormat() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 26)
}

func (x *Qualification) ClearCreatedAt() {
	x.xxx_hidden_CreatedAt = nil
}

func (x *Qualification) ClearUpdatedAt() {
	x.xxx_hidden_UpdatedAt = nil
}

func (x *Qualification) ClearDeletedAt() {
	x.xxx_hidden_DeletedAt = nil
}

func (x *Qualification) ClearDescription() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 11)
	x.xxx_hidden_Description = nil
}

func (x *Qualification) ClearContent() {
	x.xxx_hidden_Content = nil
}

func (x *Qualification) ClearCreatorId() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 13)
	x.xxx_hidden_CreatorId = 0
}

func (x *Qualification) ClearCreator() {
	x.xxx_hidden_Creator = nil
}

func (x *Qualification) ClearAccess() {
	x.xxx_hidden_Access = nil
}

func (x *Qualification) ClearDiscordSettings() {
	x.xxx_hidden_DiscordSettings = nil
}

func (x *Qualification) ClearExamSettings() {
	x.xxx_hidden_ExamSettings = nil
}

func (x *Qualification) ClearExam() {
	x.xxx_hidden_Exam = nil
}

func (x *Qualification) ClearResult() {
	x.xxx_hidden_Result = nil
}

func (x *Qualification) ClearRequest() {
	x.xxx_hidden_Request = nil
}

func (x *Qualification) ClearLabelSyncFormat() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 26)
	x.xxx_hidden_LabelSyncFormat = nil
}

type Qualification_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Id                 int64
	CreatedAt          *timestamp.Timestamp
	UpdatedAt          *timestamp.Timestamp
	DeletedAt          *timestamp.Timestamp
	Job                string
	Weight             uint32
	Closed             bool
	Draft              bool
	Public             bool
	Abbreviation       string
	Title              string
	Description        *string
	Content            *content.Content
	CreatorId          *int32
	Creator            *short.UserShort
	CreatorJob         string
	Access             *access.QualificationAccess
	Requirements       []*QualificationRequirement
	DiscordSyncEnabled bool
	DiscordSettings    *QualificationDiscordSettings
	ExamMode           exam.QualificationExamMode
	ExamSettings       *exam.QualificationExamSettings
	Exam               *exam.ExamQuestions
	Result             *QualificationResult
	Request            *QualificationRequest
	LabelSyncEnabled   bool
	LabelSyncFormat    *string
	Files              []*file.File
}

func (b0 Qualification_builder) Build() *Qualification {
	m0 := &Qualification{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Id = b.Id
	x.xxx_hidden_CreatedAt = b.CreatedAt
	x.xxx_hidden_UpdatedAt = b.UpdatedAt
	x.xxx_hidden_DeletedAt = b.DeletedAt
	x.xxx_hidden_Job = b.Job
	x.xxx_hidden_Weight = b.Weight
	x.xxx_hidden_Closed = b.Closed
	x.xxx_hidden_Draft = b.Draft
	x.xxx_hidden_Public = b.Public
	x.xxx_hidden_Abbreviation = b.Abbreviation
	x.xxx_hidden_Title = b.Title
	if b.Description != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 11, 28)
		x.xxx_hidden_Description = b.Description
	}
	x.xxx_hidden_Content = b.Content
	if b.CreatorId != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 13, 28)
		x.xxx_hidden_CreatorId = *b.CreatorId
	}
	x.xxx_hidden_Creator = b.Creator
	x.xxx_hidden_CreatorJob = b.CreatorJob
	x.xxx_hidden_Access = b.Access
	x.xxx_hidden_Requirements = &b.Requirements
	x.xxx_hidden_DiscordSyncEnabled = b.DiscordSyncEnabled
	x.xxx_hidden_DiscordSettings = b.DiscordSettings
	x.xxx_hidden_ExamMode = b.ExamMode
	x.xxx_hidden_ExamSettings = b.ExamSettings
	x.xxx_hidden_Exam = b.Exam
	x.xxx_hidden_Result = b.Result
	x.xxx_hidden_Request = b.Request
	x.xxx_hidden_LabelSyncEnabled = b.LabelSyncEnabled
	if b.LabelSyncFormat != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 26, 28)
		x.xxx_hidden_LabelSyncFormat = b.LabelSyncFormat
	}
	x.xxx_hidden_Files = &b.Files
	return m0
}

type QualificationShort struct {
	state                   protoimpl.MessageState          `protogen:"opaque.v1"`
	xxx_hidden_Id           int64                           `protobuf:"varint,1,opt,name=id,proto3"`
	xxx_hidden_CreatedAt    *timestamp.Timestamp            `protobuf:"bytes,2,opt,name=created_at,json=createdAt,proto3,oneof"`
	xxx_hidden_UpdatedAt    *timestamp.Timestamp            `protobuf:"bytes,3,opt,name=updated_at,json=updatedAt,proto3,oneof"`
	xxx_hidden_DeletedAt    *timestamp.Timestamp            `protobuf:"bytes,4,opt,name=deleted_at,json=deletedAt,proto3,oneof"`
	xxx_hidden_Job          string                          `protobuf:"bytes,5,opt,name=job,proto3"`
	xxx_hidden_Weight       uint32                          `protobuf:"varint,6,opt,name=weight,proto3"`
	xxx_hidden_Closed       bool                            `protobuf:"varint,7,opt,name=closed,proto3"`
	xxx_hidden_Draft        bool                            `protobuf:"varint,8,opt,name=draft,proto3"`
	xxx_hidden_Public       bool                            `protobuf:"varint,9,opt,name=public,proto3"`
	xxx_hidden_Abbreviation string                          `protobuf:"bytes,10,opt,name=abbreviation,proto3"`
	xxx_hidden_Title        string                          `protobuf:"bytes,11,opt,name=title,proto3"`
	xxx_hidden_Description  *string                         `protobuf:"bytes,12,opt,name=description,proto3,oneof"`
	xxx_hidden_CreatorId    int32                           `protobuf:"varint,14,opt,name=creator_id,json=creatorId,proto3,oneof"`
	xxx_hidden_Creator      *short.UserShort                `protobuf:"bytes,15,opt,name=creator,proto3,oneof"`
	xxx_hidden_CreatorJob   string                          `protobuf:"bytes,16,opt,name=creator_job,json=creatorJob,proto3"`
	xxx_hidden_Requirements *[]*QualificationRequirement    `protobuf:"bytes,18,rep,name=requirements,proto3"`
	xxx_hidden_ExamMode     exam.QualificationExamMode      `protobuf:"varint,21,opt,name=exam_mode,json=examMode,proto3,enum=resources.qualifications.exam.QualificationExamMode"`
	xxx_hidden_ExamSettings *exam.QualificationExamSettings `protobuf:"bytes,22,opt,name=exam_settings,json=examSettings,proto3,oneof"`
	xxx_hidden_Result       *QualificationResult            `protobuf:"bytes,24,opt,name=result,proto3,oneof"`
	XXX_raceDetectHookData  protoimpl.RaceDetectHookData
	XXX_presence            [1]uint32
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *QualificationShort) Reset() {
	*x = QualificationShort{}
	mi := &file_resources_qualifications_qualifications_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QualificationShort) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QualificationShort) ProtoMessage() {}

func (x *QualificationShort) ProtoReflect() protoreflect.Message {
	mi := &file_resources_qualifications_qualifications_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *QualificationShort) GetId() int64 {
	if x != nil {
		return x.xxx_hidden_Id
	}
	return 0
}

func (x *QualificationShort) GetCreatedAt() *timestamp.Timestamp {
	if x != nil {
		return x.xxx_hidden_CreatedAt
	}
	return nil
}

func (x *QualificationShort) GetUpdatedAt() *timestamp.Timestamp {
	if x != nil {
		return x.xxx_hidden_UpdatedAt
	}
	return nil
}

func (x *QualificationShort) GetDeletedAt() *timestamp.Timestamp {
	if x != nil {
		return x.xxx_hidden_DeletedAt
	}
	return nil
}

func (x *QualificationShort) GetJob() string {
	if x != nil {
		return x.xxx_hidden_Job
	}
	return ""
}

func (x *QualificationShort) GetWeight() uint32 {
	if x != nil {
		return x.xxx_hidden_Weight
	}
	return 0
}

func (x *QualificationShort) GetClosed() bool {
	if x != nil {
		return x.xxx_hidden_Closed
	}
	return false
}

func (x *QualificationShort) GetDraft() bool {
	if x != nil {
		return x.xxx_hidden_Draft
	}
	return false
}

func (x *QualificationShort) GetPublic() bool {
	if x != nil {
		return x.xxx_hidden_Public
	}
	return false
}

func (x *QualificationShort) GetAbbreviation() string {
	if x != nil {
		return x.xxx_hidden_Abbreviation
	}
	return ""
}

func (x *QualificationShort) GetTitle() string {
	if x != nil {
		return x.xxx_hidden_Title
	}
	return ""
}

func (x *QualificationShort) GetDescription() string {
	if x != nil {
		if x.xxx_hidden_Description != nil {
			return *x.xxx_hidden_Description
		}
		return ""
	}
	return ""
}

func (x *QualificationShort) GetCreatorId() int32 {
	if x != nil {
		return x.xxx_hidden_CreatorId
	}
	return 0
}

func (x *QualificationShort) GetCreator() *short.UserShort {
	if x != nil {
		return x.xxx_hidden_Creator
	}
	return nil
}

func (x *QualificationShort) GetCreatorJob() string {
	if x != nil {
		return x.xxx_hidden_CreatorJob
	}
	return ""
}

func (x *QualificationShort) GetRequirements() []*QualificationRequirement {
	if x != nil {
		if x.xxx_hidden_Requirements != nil {
			return *x.xxx_hidden_Requirements
		}
	}
	return nil
}

func (x *QualificationShort) GetExamMode() exam.QualificationExamMode {
	if x != nil {
		return x.xxx_hidden_ExamMode
	}
	return exam.QualificationExamMode(0)
}

func (x *QualificationShort) GetExamSettings() *exam.QualificationExamSettings {
	if x != nil {
		return x.xxx_hidden_ExamSettings
	}
	return nil
}

func (x *QualificationShort) GetResult() *QualificationResult {
	if x != nil {
		return x.xxx_hidden_Result
	}
	return nil
}

func (x *QualificationShort) SetId(v int64) {
	x.xxx_hidden_Id = v
}

func (x *QualificationShort) SetCreatedAt(v *timestamp.Timestamp) {
	x.xxx_hidden_CreatedAt = v
}

func (x *QualificationShort) SetUpdatedAt(v *timestamp.Timestamp) {
	x.xxx_hidden_UpdatedAt = v
}

func (x *QualificationShort) SetDeletedAt(v *timestamp.Timestamp) {
	x.xxx_hidden_DeletedAt = v
}

func (x *QualificationShort) SetJob(v string) {
	x.xxx_hidden_Job = v
}

func (x *QualificationShort) SetWeight(v uint32) {
	x.xxx_hidden_Weight = v
}

func (x *QualificationShort) SetClosed(v bool) {
	x.xxx_hidden_Closed = v
}

func (x *QualificationShort) SetDraft(v bool) {
	x.xxx_hidden_Draft = v
}

func (x *QualificationShort) SetPublic(v bool) {
	x.xxx_hidden_Public = v
}

func (x *QualificationShort) SetAbbreviation(v string) {
	x.xxx_hidden_Abbreviation = v
}

func (x *QualificationShort) SetTitle(v string) {
	x.xxx_hidden_Title = v
}

func (x *QualificationShort) SetDescription(v string) {
	x.xxx_hidden_Description = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 11, 19)
}

func (x *QualificationShort) SetCreatorId(v int32) {
	x.xxx_hidden_CreatorId = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 12, 19)
}

func (x *QualificationShort) SetCreator(v *short.UserShort) {
	x.xxx_hidden_Creator = v
}

func (x *QualificationShort) SetCreatorJob(v string) {
	x.xxx_hidden_CreatorJob = v
}

func (x *QualificationShort) SetRequirements(v []*QualificationRequirement) {
	x.xxx_hidden_Requirements = &v
}

func (x *QualificationShort) SetExamMode(v exam.QualificationExamMode) {
	x.xxx_hidden_ExamMode = v
}

func (x *QualificationShort) SetExamSettings(v *exam.QualificationExamSettings) {
	x.xxx_hidden_ExamSettings = v
}

func (x *QualificationShort) SetResult(v *QualificationResult) {
	x.xxx_hidden_Result = v
}

func (x *QualificationShort) HasCreatedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_CreatedAt != nil
}

func (x *QualificationShort) HasUpdatedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_UpdatedAt != nil
}

func (x *QualificationShort) HasDeletedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_DeletedAt != nil
}

func (x *QualificationShort) HasDescription() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 11)
}

func (x *QualificationShort) HasCreatorId() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 12)
}

func (x *QualificationShort) HasCreator() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Creator != nil
}

func (x *QualificationShort) HasExamSettings() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_ExamSettings != nil
}

func (x *QualificationShort) HasResult() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Result != nil
}

func (x *QualificationShort) ClearCreatedAt() {
	x.xxx_hidden_CreatedAt = nil
}

func (x *QualificationShort) ClearUpdatedAt() {
	x.xxx_hidden_UpdatedAt = nil
}

func (x *QualificationShort) ClearDeletedAt() {
	x.xxx_hidden_DeletedAt = nil
}

func (x *QualificationShort) ClearDescription() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 11)
	x.xxx_hidden_Description = nil
}

func (x *QualificationShort) ClearCreatorId() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 12)
	x.xxx_hidden_CreatorId = 0
}

func (x *QualificationShort) ClearCreator() {
	x.xxx_hidden_Creator = nil
}

func (x *QualificationShort) ClearExamSettings() {
	x.xxx_hidden_ExamSettings = nil
}

func (x *QualificationShort) ClearResult() {
	x.xxx_hidden_Result = nil
}

type QualificationShort_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Id           int64
	CreatedAt    *timestamp.Timestamp
	UpdatedAt    *timestamp.Timestamp
	DeletedAt    *timestamp.Timestamp
	Job          string
	Weight       uint32
	Closed       bool
	Draft        bool
	Public       bool
	Abbreviation string
	Title        string
	Description  *string
	CreatorId    *int32
	Creator      *short.UserShort
	CreatorJob   string
	Requirements []*QualificationRequirement
	ExamMode     exam.QualificationExamMode
	ExamSettings *exam.QualificationExamSettings
	Result       *QualificationResult
}

func (b0 QualificationShort_builder) Build() *QualificationShort {
	m0 := &QualificationShort{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Id = b.Id
	x.xxx_hidden_CreatedAt = b.CreatedAt
	x.xxx_hidden_UpdatedAt = b.UpdatedAt
	x.xxx_hidden_DeletedAt = b.DeletedAt
	x.xxx_hidden_Job = b.Job
	x.xxx_hidden_Weight = b.Weight
	x.xxx_hidden_Closed = b.Closed
	x.xxx_hidden_Draft = b.Draft
	x.xxx_hidden_Public = b.Public
	x.xxx_hidden_Abbreviation = b.Abbreviation
	x.xxx_hidden_Title = b.Title
	if b.Description != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 11, 19)
		x.xxx_hidden_Description = b.Description
	}
	if b.CreatorId != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 12, 19)
		x.xxx_hidden_CreatorId = *b.CreatorId
	}
	x.xxx_hidden_Creator = b.Creator
	x.xxx_hidden_CreatorJob = b.CreatorJob
	x.xxx_hidden_Requirements = &b.Requirements
	x.xxx_hidden_ExamMode = b.ExamMode
	x.xxx_hidden_ExamSettings = b.ExamSettings
	x.xxx_hidden_Result = b.Result
	return m0
}

type QualificationRequirement struct {
	state                            protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Id                    int64                  `protobuf:"varint,1,opt,name=id,proto3"`
	xxx_hidden_CreatedAt             *timestamp.Timestamp   `protobuf:"bytes,2,opt,name=created_at,json=createdAt,proto3,oneof"`
	xxx_hidden_QualificationId       int64                  `protobuf:"varint,3,opt,name=qualification_id,json=qualificationId,proto3"`
	xxx_hidden_TargetQualificationId int64                  `protobuf:"varint,4,opt,name=target_qualification_id,json=targetQualificationId,proto3"`
	xxx_hidden_TargetQualification   *QualificationShort    `protobuf:"bytes,5,opt,name=target_qualification,json=targetQualification,proto3,oneof"`
	unknownFields                    protoimpl.UnknownFields
	sizeCache                        protoimpl.SizeCache
}

func (x *QualificationRequirement) Reset() {
	*x = QualificationRequirement{}
	mi := &file_resources_qualifications_qualifications_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QualificationRequirement) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QualificationRequirement) ProtoMessage() {}

func (x *QualificationRequirement) ProtoReflect() protoreflect.Message {
	mi := &file_resources_qualifications_qualifications_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *QualificationRequirement) GetId() int64 {
	if x != nil {
		return x.xxx_hidden_Id
	}
	return 0
}

func (x *QualificationRequirement) GetCreatedAt() *timestamp.Timestamp {
	if x != nil {
		return x.xxx_hidden_CreatedAt
	}
	return nil
}

func (x *QualificationRequirement) GetQualificationId() int64 {
	if x != nil {
		return x.xxx_hidden_QualificationId
	}
	return 0
}

func (x *QualificationRequirement) GetTargetQualificationId() int64 {
	if x != nil {
		return x.xxx_hidden_TargetQualificationId
	}
	return 0
}

func (x *QualificationRequirement) GetTargetQualification() *QualificationShort {
	if x != nil {
		return x.xxx_hidden_TargetQualification
	}
	return nil
}

func (x *QualificationRequirement) SetId(v int64) {
	x.xxx_hidden_Id = v
}

func (x *QualificationRequirement) SetCreatedAt(v *timestamp.Timestamp) {
	x.xxx_hidden_CreatedAt = v
}

func (x *QualificationRequirement) SetQualificationId(v int64) {
	x.xxx_hidden_QualificationId = v
}

func (x *QualificationRequirement) SetTargetQualificationId(v int64) {
	x.xxx_hidden_TargetQualificationId = v
}

func (x *QualificationRequirement) SetTargetQualification(v *QualificationShort) {
	x.xxx_hidden_TargetQualification = v
}

func (x *QualificationRequirement) HasCreatedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_CreatedAt != nil
}

func (x *QualificationRequirement) HasTargetQualification() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_TargetQualification != nil
}

func (x *QualificationRequirement) ClearCreatedAt() {
	x.xxx_hidden_CreatedAt = nil
}

func (x *QualificationRequirement) ClearTargetQualification() {
	x.xxx_hidden_TargetQualification = nil
}

type QualificationRequirement_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Id                    int64
	CreatedAt             *timestamp.Timestamp
	QualificationId       int64
	TargetQualificationId int64
	TargetQualification   *QualificationShort
}

func (b0 QualificationRequirement_builder) Build() *QualificationRequirement {
	m0 := &QualificationRequirement{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Id = b.Id
	x.xxx_hidden_CreatedAt = b.CreatedAt
	x.xxx_hidden_QualificationId = b.QualificationId
	x.xxx_hidden_TargetQualificationId = b.TargetQualificationId
	x.xxx_hidden_TargetQualification = b.TargetQualification
	return m0
}

type QualificationDiscordSettings struct {
	state                  protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_RoleName    *string                `protobuf:"bytes,1,opt,name=role_name,json=roleName,proto3,oneof"`
	xxx_hidden_RoleFormat  *string                `protobuf:"bytes,2,opt,name=role_format,json=roleFormat,proto3,oneof"`
	XXX_raceDetectHookData protoimpl.RaceDetectHookData
	XXX_presence           [1]uint32
	unknownFields          protoimpl.UnknownFields
	sizeCache              protoimpl.SizeCache
}

func (x *QualificationDiscordSettings) Reset() {
	*x = QualificationDiscordSettings{}
	mi := &file_resources_qualifications_qualifications_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QualificationDiscordSettings) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QualificationDiscordSettings) ProtoMessage() {}

func (x *QualificationDiscordSettings) ProtoReflect() protoreflect.Message {
	mi := &file_resources_qualifications_qualifications_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *QualificationDiscordSettings) GetRoleName() string {
	if x != nil {
		if x.xxx_hidden_RoleName != nil {
			return *x.xxx_hidden_RoleName
		}
		return ""
	}
	return ""
}

func (x *QualificationDiscordSettings) GetRoleFormat() string {
	if x != nil {
		if x.xxx_hidden_RoleFormat != nil {
			return *x.xxx_hidden_RoleFormat
		}
		return ""
	}
	return ""
}

func (x *QualificationDiscordSettings) SetRoleName(v string) {
	x.xxx_hidden_RoleName = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 0, 2)
}

func (x *QualificationDiscordSettings) SetRoleFormat(v string) {
	x.xxx_hidden_RoleFormat = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 1, 2)
}

func (x *QualificationDiscordSettings) HasRoleName() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 0)
}

func (x *QualificationDiscordSettings) HasRoleFormat() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 1)
}

func (x *QualificationDiscordSettings) ClearRoleName() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 0)
	x.xxx_hidden_RoleName = nil
}

func (x *QualificationDiscordSettings) ClearRoleFormat() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 1)
	x.xxx_hidden_RoleFormat = nil
}

type QualificationDiscordSettings_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	RoleName   *string
	RoleFormat *string
}

func (b0 QualificationDiscordSettings_builder) Build() *QualificationDiscordSettings {
	m0 := &QualificationDiscordSettings{}
	b, x := &b0, m0
	_, _ = b, x
	if b.RoleName != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 0, 2)
		x.xxx_hidden_RoleName = b.RoleName
	}
	if b.RoleFormat != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 1, 2)
		x.xxx_hidden_RoleFormat = b.RoleFormat
	}
	return m0
}

type QualificationRequest struct {
	state                      protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_CreatedAt       *timestamp.Timestamp   `protobuf:"bytes,1,opt,name=created_at,json=createdAt,proto3,oneof"`
	xxx_hidden_DeletedAt       *timestamp.Timestamp   `protobuf:"bytes,2,opt,name=deleted_at,json=deletedAt,proto3,oneof"`
	xxx_hidden_QualificationId int64                  `protobuf:"varint,3,opt,name=qualification_id,json=qualificationId,proto3"`
	xxx_hidden_Qualification   *QualificationShort    `protobuf:"bytes,4,opt,name=qualification,proto3,oneof"`
	xxx_hidden_UserId          int32                  `protobuf:"varint,5,opt,name=user_id,json=userId,proto3"`
	xxx_hidden_User            *short.UserShort       `protobuf:"bytes,6,opt,name=user,proto3"`
	xxx_hidden_UserComment     *string                `protobuf:"bytes,7,opt,name=user_comment,json=userComment,proto3,oneof"`
	xxx_hidden_Status          RequestStatus          `protobuf:"varint,8,opt,name=status,proto3,enum=resources.qualifications.RequestStatus,oneof"`
	xxx_hidden_ApprovedAt      *timestamp.Timestamp   `protobuf:"bytes,9,opt,name=approved_at,json=approvedAt,proto3,oneof"`
	xxx_hidden_ApproverComment *string                `protobuf:"bytes,10,opt,name=approver_comment,json=approverComment,proto3,oneof"`
	xxx_hidden_ApproverId      int32                  `protobuf:"varint,11,opt,name=approver_id,json=approverId,proto3,oneof"`
	xxx_hidden_Approver        *short.UserShort       `protobuf:"bytes,12,opt,name=approver,proto3,oneof"`
	xxx_hidden_ApproverJob     *string                `protobuf:"bytes,13,opt,name=approver_job,json=approverJob,proto3,oneof"`
	XXX_raceDetectHookData     protoimpl.RaceDetectHookData
	XXX_presence               [1]uint32
	unknownFields              protoimpl.UnknownFields
	sizeCache                  protoimpl.SizeCache
}

func (x *QualificationRequest) Reset() {
	*x = QualificationRequest{}
	mi := &file_resources_qualifications_qualifications_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QualificationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QualificationRequest) ProtoMessage() {}

func (x *QualificationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_resources_qualifications_qualifications_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *QualificationRequest) GetCreatedAt() *timestamp.Timestamp {
	if x != nil {
		return x.xxx_hidden_CreatedAt
	}
	return nil
}

func (x *QualificationRequest) GetDeletedAt() *timestamp.Timestamp {
	if x != nil {
		return x.xxx_hidden_DeletedAt
	}
	return nil
}

func (x *QualificationRequest) GetQualificationId() int64 {
	if x != nil {
		return x.xxx_hidden_QualificationId
	}
	return 0
}

func (x *QualificationRequest) GetQualification() *QualificationShort {
	if x != nil {
		return x.xxx_hidden_Qualification
	}
	return nil
}

func (x *QualificationRequest) GetUserId() int32 {
	if x != nil {
		return x.xxx_hidden_UserId
	}
	return 0
}

func (x *QualificationRequest) GetUser() *short.UserShort {
	if x != nil {
		return x.xxx_hidden_User
	}
	return nil
}

func (x *QualificationRequest) GetUserComment() string {
	if x != nil {
		if x.xxx_hidden_UserComment != nil {
			return *x.xxx_hidden_UserComment
		}
		return ""
	}
	return ""
}

func (x *QualificationRequest) GetStatus() RequestStatus {
	if x != nil {
		if protoimpl.X.Present(&(x.XXX_presence[0]), 7) {
			return x.xxx_hidden_Status
		}
	}
	return RequestStatus_REQUEST_STATUS_UNSPECIFIED
}

func (x *QualificationRequest) GetApprovedAt() *timestamp.Timestamp {
	if x != nil {
		return x.xxx_hidden_ApprovedAt
	}
	return nil
}

func (x *QualificationRequest) GetApproverComment() string {
	if x != nil {
		if x.xxx_hidden_ApproverComment != nil {
			return *x.xxx_hidden_ApproverComment
		}
		return ""
	}
	return ""
}

func (x *QualificationRequest) GetApproverId() int32 {
	if x != nil {
		return x.xxx_hidden_ApproverId
	}
	return 0
}

func (x *QualificationRequest) GetApprover() *short.UserShort {
	if x != nil {
		return x.xxx_hidden_Approver
	}
	return nil
}

func (x *QualificationRequest) GetApproverJob() string {
	if x != nil {
		if x.xxx_hidden_ApproverJob != nil {
			return *x.xxx_hidden_ApproverJob
		}
		return ""
	}
	return ""
}

func (x *QualificationRequest) SetCreatedAt(v *timestamp.Timestamp) {
	x.xxx_hidden_CreatedAt = v
}

func (x *QualificationRequest) SetDeletedAt(v *timestamp.Timestamp) {
	x.xxx_hidden_DeletedAt = v
}

func (x *QualificationRequest) SetQualificationId(v int64) {
	x.xxx_hidden_QualificationId = v
}

func (x *QualificationRequest) SetQualification(v *QualificationShort) {
	x.xxx_hidden_Qualification = v
}

func (x *QualificationRequest) SetUserId(v int32) {
	x.xxx_hidden_UserId = v
}

func (x *QualificationRequest) SetUser(v *short.UserShort) {
	x.xxx_hidden_User = v
}

func (x *QualificationRequest) SetUserComment(v string) {
	x.xxx_hidden_UserComment = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 6, 13)
}

func (x *QualificationRequest) SetStatus(v RequestStatus) {
	x.xxx_hidden_Status = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 7, 13)
}

func (x *QualificationRequest) SetApprovedAt(v *timestamp.Timestamp) {
	x.xxx_hidden_ApprovedAt = v
}

func (x *QualificationRequest) SetApproverComment(v string) {
	x.xxx_hidden_ApproverComment = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 9, 13)
}

func (x *QualificationRequest) SetApproverId(v int32) {
	x.xxx_hidden_ApproverId = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 10, 13)
}

func (x *QualificationRequest) SetApprover(v *short.UserShort) {
	x.xxx_hidden_Approver = v
}

func (x *QualificationRequest) SetApproverJob(v string) {
	x.xxx_hidden_ApproverJob = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 12, 13)
}

func (x *QualificationRequest) HasCreatedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_CreatedAt != nil
}

func (x *QualificationRequest) HasDeletedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_DeletedAt != nil
}

func (x *QualificationRequest) HasQualification() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Qualification != nil
}

func (x *QualificationRequest) HasUser() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_User != nil
}

func (x *QualificationRequest) HasUserComment() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 6)
}

func (x *QualificationRequest) HasStatus() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 7)
}

func (x *QualificationRequest) HasApprovedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_ApprovedAt != nil
}

func (x *QualificationRequest) HasApproverComment() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 9)
}

func (x *QualificationRequest) HasApproverId() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 10)
}

func (x *QualificationRequest) HasApprover() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Approver != nil
}

func (x *QualificationRequest) HasApproverJob() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 12)
}

func (x *QualificationRequest) ClearCreatedAt() {
	x.xxx_hidden_CreatedAt = nil
}

func (x *QualificationRequest) ClearDeletedAt() {
	x.xxx_hidden_DeletedAt = nil
}

func (x *QualificationRequest) ClearQualification() {
	x.xxx_hidden_Qualification = nil
}

func (x *QualificationRequest) ClearUser() {
	x.xxx_hidden_User = nil
}

func (x *QualificationRequest) ClearUserComment() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 6)
	x.xxx_hidden_UserComment = nil
}

func (x *QualificationRequest) ClearStatus() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 7)
	x.xxx_hidden_Status = RequestStatus_REQUEST_STATUS_UNSPECIFIED
}

func (x *QualificationRequest) ClearApprovedAt() {
	x.xxx_hidden_ApprovedAt = nil
}

func (x *QualificationRequest) ClearApproverComment() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 9)
	x.xxx_hidden_ApproverComment = nil
}

func (x *QualificationRequest) ClearApproverId() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 10)
	x.xxx_hidden_ApproverId = 0
}

func (x *QualificationRequest) ClearApprover() {
	x.xxx_hidden_Approver = nil
}

func (x *QualificationRequest) ClearApproverJob() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 12)
	x.xxx_hidden_ApproverJob = nil
}

type QualificationRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	CreatedAt       *timestamp.Timestamp
	DeletedAt       *timestamp.Timestamp
	QualificationId int64
	Qualification   *QualificationShort
	UserId          int32
	User            *short.UserShort
	UserComment     *string
	Status          *RequestStatus
	ApprovedAt      *timestamp.Timestamp
	ApproverComment *string
	ApproverId      *int32
	Approver        *short.UserShort
	ApproverJob     *string
}

func (b0 QualificationRequest_builder) Build() *QualificationRequest {
	m0 := &QualificationRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_CreatedAt = b.CreatedAt
	x.xxx_hidden_DeletedAt = b.DeletedAt
	x.xxx_hidden_QualificationId = b.QualificationId
	x.xxx_hidden_Qualification = b.Qualification
	x.xxx_hidden_UserId = b.UserId
	x.xxx_hidden_User = b.User
	if b.UserComment != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 6, 13)
		x.xxx_hidden_UserComment = b.UserComment
	}
	if b.Status != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 7, 13)
		x.xxx_hidden_Status = *b.Status
	}
	x.xxx_hidden_ApprovedAt = b.ApprovedAt
	if b.ApproverComment != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 9, 13)
		x.xxx_hidden_ApproverComment = b.ApproverComment
	}
	if b.ApproverId != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 10, 13)
		x.xxx_hidden_ApproverId = *b.ApproverId
	}
	x.xxx_hidden_Approver = b.Approver
	if b.ApproverJob != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 12, 13)
		x.xxx_hidden_ApproverJob = b.ApproverJob
	}
	return m0
}

type QualificationResult struct {
	state                      protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Id              int64                  `protobuf:"varint,1,opt,name=id,proto3"`
	xxx_hidden_CreatedAt       *timestamp.Timestamp   `protobuf:"bytes,2,opt,name=created_at,json=createdAt,proto3,oneof"`
	xxx_hidden_DeletedAt       *timestamp.Timestamp   `protobuf:"bytes,3,opt,name=deleted_at,json=deletedAt,proto3,oneof"`
	xxx_hidden_QualificationId int64                  `protobuf:"varint,4,opt,name=qualification_id,json=qualificationId,proto3"`
	xxx_hidden_Qualification   *QualificationShort    `protobuf:"bytes,5,opt,name=qualification,proto3,oneof"`
	xxx_hidden_UserId          int32                  `protobuf:"varint,6,opt,name=user_id,json=userId,proto3"`
	xxx_hidden_User            *short.UserShort       `protobuf:"bytes,7,opt,name=user,proto3"`
	xxx_hidden_Status          ResultStatus           `protobuf:"varint,8,opt,name=status,proto3,enum=resources.qualifications.ResultStatus"`
	xxx_hidden_Score           float32                `protobuf:"fixed32,9,opt,name=score,proto3,oneof"`
	xxx_hidden_Summary         string                 `protobuf:"bytes,10,opt,name=summary,proto3"`
	xxx_hidden_CreatorId       int32                  `protobuf:"varint,11,opt,name=creator_id,json=creatorId,proto3"`
	xxx_hidden_Creator         *short.UserShort       `protobuf:"bytes,12,opt,name=creator,proto3"`
	xxx_hidden_CreatorJob      string                 `protobuf:"bytes,13,opt,name=creator_job,json=creatorJob,proto3"`
	XXX_raceDetectHookData     protoimpl.RaceDetectHookData
	XXX_presence               [1]uint32
	unknownFields              protoimpl.UnknownFields
	sizeCache                  protoimpl.SizeCache
}

func (x *QualificationResult) Reset() {
	*x = QualificationResult{}
	mi := &file_resources_qualifications_qualifications_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QualificationResult) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QualificationResult) ProtoMessage() {}

func (x *QualificationResult) ProtoReflect() protoreflect.Message {
	mi := &file_resources_qualifications_qualifications_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *QualificationResult) GetId() int64 {
	if x != nil {
		return x.xxx_hidden_Id
	}
	return 0
}

func (x *QualificationResult) GetCreatedAt() *timestamp.Timestamp {
	if x != nil {
		return x.xxx_hidden_CreatedAt
	}
	return nil
}

func (x *QualificationResult) GetDeletedAt() *timestamp.Timestamp {
	if x != nil {
		return x.xxx_hidden_DeletedAt
	}
	return nil
}

func (x *QualificationResult) GetQualificationId() int64 {
	if x != nil {
		return x.xxx_hidden_QualificationId
	}
	return 0
}

func (x *QualificationResult) GetQualification() *QualificationShort {
	if x != nil {
		return x.xxx_hidden_Qualification
	}
	return nil
}

func (x *QualificationResult) GetUserId() int32 {
	if x != nil {
		return x.xxx_hidden_UserId
	}
	return 0
}

func (x *QualificationResult) GetUser() *short.UserShort {
	if x != nil {
		return x.xxx_hidden_User
	}
	return nil
}

func (x *QualificationResult) GetStatus() ResultStatus {
	if x != nil {
		return x.xxx_hidden_Status
	}
	return ResultStatus_RESULT_STATUS_UNSPECIFIED
}

func (x *QualificationResult) GetScore() float32 {
	if x != nil {
		return x.xxx_hidden_Score
	}
	return 0
}

func (x *QualificationResult) GetSummary() string {
	if x != nil {
		return x.xxx_hidden_Summary
	}
	return ""
}

func (x *QualificationResult) GetCreatorId() int32 {
	if x != nil {
		return x.xxx_hidden_CreatorId
	}
	return 0
}

func (x *QualificationResult) GetCreator() *short.UserShort {
	if x != nil {
		return x.xxx_hidden_Creator
	}
	return nil
}

func (x *QualificationResult) GetCreatorJob() string {
	if x != nil {
		return x.xxx_hidden_CreatorJob
	}
	return ""
}

func (x *QualificationResult) SetId(v int64) {
	x.xxx_hidden_Id = v
}

func (x *QualificationResult) SetCreatedAt(v *timestamp.Timestamp) {
	x.xxx_hidden_CreatedAt = v
}

func (x *QualificationResult) SetDeletedAt(v *timestamp.Timestamp) {
	x.xxx_hidden_DeletedAt = v
}

func (x *QualificationResult) SetQualificationId(v int64) {
	x.xxx_hidden_QualificationId = v
}

func (x *QualificationResult) SetQualification(v *QualificationShort) {
	x.xxx_hidden_Qualification = v
}

func (x *QualificationResult) SetUserId(v int32) {
	x.xxx_hidden_UserId = v
}

func (x *QualificationResult) SetUser(v *short.UserShort) {
	x.xxx_hidden_User = v
}

func (x *QualificationResult) SetStatus(v ResultStatus) {
	x.xxx_hidden_Status = v
}

func (x *QualificationResult) SetScore(v float32) {
	x.xxx_hidden_Score = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 8, 13)
}

func (x *QualificationResult) SetSummary(v string) {
	x.xxx_hidden_Summary = v
}

func (x *QualificationResult) SetCreatorId(v int32) {
	x.xxx_hidden_CreatorId = v
}

func (x *QualificationResult) SetCreator(v *short.UserShort) {
	x.xxx_hidden_Creator = v
}

func (x *QualificationResult) SetCreatorJob(v string) {
	x.xxx_hidden_CreatorJob = v
}

func (x *QualificationResult) HasCreatedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_CreatedAt != nil
}

func (x *QualificationResult) HasDeletedAt() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_DeletedAt != nil
}

func (x *QualificationResult) HasQualification() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Qualification != nil
}

func (x *QualificationResult) HasUser() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_User != nil
}

func (x *QualificationResult) HasScore() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 8)
}

func (x *QualificationResult) HasCreator() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Creator != nil
}

func (x *QualificationResult) ClearCreatedAt() {
	x.xxx_hidden_CreatedAt = nil
}

func (x *QualificationResult) ClearDeletedAt() {
	x.xxx_hidden_DeletedAt = nil
}

func (x *QualificationResult) ClearQualification() {
	x.xxx_hidden_Qualification = nil
}

func (x *QualificationResult) ClearUser() {
	x.xxx_hidden_User = nil
}

func (x *QualificationResult) ClearScore() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 8)
	x.xxx_hidden_Score = 0
}

func (x *QualificationResult) ClearCreator() {
	x.xxx_hidden_Creator = nil
}

type QualificationResult_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	Id              int64
	CreatedAt       *timestamp.Timestamp
	DeletedAt       *timestamp.Timestamp
	QualificationId int64
	Qualification   *QualificationShort
	UserId          int32
	User            *short.UserShort
	Status          ResultStatus
	Score           *float32
	Summary         string
	CreatorId       int32
	Creator         *short.UserShort
	CreatorJob      string
}

func (b0 QualificationResult_builder) Build() *QualificationResult {
	m0 := &QualificationResult{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Id = b.Id
	x.xxx_hidden_CreatedAt = b.CreatedAt
	x.xxx_hidden_DeletedAt = b.DeletedAt
	x.xxx_hidden_QualificationId = b.QualificationId
	x.xxx_hidden_Qualification = b.Qualification
	x.xxx_hidden_UserId = b.UserId
	x.xxx_hidden_User = b.User
	x.xxx_hidden_Status = b.Status
	if b.Score != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 8, 13)
		x.xxx_hidden_Score = *b.Score
	}
	x.xxx_hidden_Summary = b.Summary
	x.xxx_hidden_CreatorId = b.CreatorId
	x.xxx_hidden_Creator = b.Creator
	x.xxx_hidden_CreatorJob = b.CreatorJob
	return m0
}

var File_resources_qualifications_qualifications_proto protoreflect.FileDescriptor

const file_resources_qualifications_qualifications_proto_rawDesc = "" +
	"\n" +
	"-resources/qualifications/qualifications.proto\x12\x18resources.qualifications\x1a!codegen/dbscanner/dbscanner.proto\x1a!codegen/sanitizer/sanitizer.proto\x1a&resources/common/content/content.proto\x1a\x19resources/file/file.proto\x1a,resources/qualifications/access/access.proto\x1a(resources/qualifications/exam/exam.proto\x1a#resources/timestamp/timestamp.proto\x1a resources/users/short/user.proto\x1a\x13tagger/tagger.proto\"\xe6\x0e\n" +
	"\rQualification\x121\n" +
	"\x02id\x18\x01 \x01(\x03B!\x9a\x84\x9e\x03\x1csql:\"primary_key\" alias:\"id\"R\x02id\x12B\n" +
	"\n" +
	"created_at\x18\x02 \x01(\v2\x1e.resources.timestamp.TimestampH\x00R\tcreatedAt\x88\x01\x01\x12B\n" +
	"\n" +
	"updated_at\x18\x03 \x01(\v2\x1e.resources.timestamp.TimestampH\x01R\tupdatedAt\x88\x01\x01\x12B\n" +
	"\n" +
	"deleted_at\x18\x04 \x01(\v2\x1e.resources.timestamp.TimestampH\x02R\tdeletedAt\x88\x01\x01\x12\x19\n" +
	"\x03job\x18\x05 \x01(\tB\a\xbaH\x04r\x02\x18\x14R\x03job\x12#\n" +
	"\x06weight\x18\x06 \x01(\rB\v\xbaH\b*\x06\x10\xff\xff\xff\xff\x0fR\x06weight\x12\x16\n" +
	"\x06closed\x18\a \x01(\bR\x06closed\x12\x14\n" +
	"\x05draft\x18\b \x01(\bR\x05draft\x12\x16\n" +
	"\x06public\x18\t \x01(\bR\x06public\x123\n" +
	"\fabbreviation\x18\n" +
	" \x01(\tB\x0f\xda\xf3\x18\x04\b\x01\x18\x01\xbaH\x04r\x02\x18\x14R\fabbreviation\x12&\n" +
	"\x05title\x18\v \x01(\tB\x10\xda\xf3\x18\x02\b\x01\xbaH\ar\x05\x10\x03\x18\x80\bR\x05title\x127\n" +
	"\vdescription\x18\f \x01(\tB\x10\xda\xf3\x18\x04\b\x01\x18\x01\xbaH\x05r\x03\x18\x80\x04H\x03R\vdescription\x88\x01\x01\x12;\n" +
	"\acontent\x18\r \x01(\v2!.resources.common.content.ContentR\acontent\x12+\n" +
	"\n" +
	"creator_id\x18\x0e \x01(\x05B\a\xbaH\x04\x1a\x02 \x00H\x04R\tcreatorId\x88\x01\x01\x12U\n" +
	"\acreator\x18\x0f \x01(\v2 .resources.users.short.UserShortB\x14\x9a\x84\x9e\x03\x0falias:\"creator\"H\x05R\acreator\x88\x01\x01\x12(\n" +
	"\vcreator_job\x18\x10 \x01(\tB\a\xbaH\x04r\x02\x18\x14R\n" +
	"creatorJob\x12L\n" +
	"\x06access\x18\x11 \x01(\v24.resources.qualifications.access.QualificationAccessR\x06access\x12V\n" +
	"\frequirements\x18\x12 \x03(\v22.resources.qualifications.QualificationRequirementR\frequirements\x120\n" +
	"\x14discord_sync_enabled\x18\x13 \x01(\bR\x12discordSyncEnabled\x12f\n" +
	"\x10discord_settings\x18\x14 \x01(\v26.resources.qualifications.QualificationDiscordSettingsH\x06R\x0fdiscordSettings\x88\x01\x01\x12[\n" +
	"\texam_mode\x18\x15 \x01(\x0e24.resources.qualifications.exam.QualificationExamModeB\b\xbaH\x05\x82\x01\x02\x10\x01R\bexamMode\x12b\n" +
	"\rexam_settings\x18\x16 \x01(\v28.resources.qualifications.exam.QualificationExamSettingsH\aR\fexamSettings\x88\x01\x01\x12E\n" +
	"\x04exam\x18\x17 \x01(\v2,.resources.qualifications.exam.ExamQuestionsH\bR\x04exam\x88\x01\x01\x12J\n" +
	"\x06result\x18\x18 \x01(\v2-.resources.qualifications.QualificationResultH\tR\x06result\x88\x01\x01\x12M\n" +
	"\arequest\x18\x19 \x01(\v2..resources.qualifications.QualificationRequestH\n" +
	"R\arequest\x88\x01\x01\x12,\n" +
	"\x12label_sync_enabled\x18\x1a \x01(\bR\x10labelSyncEnabled\x12A\n" +
	"\x11label_sync_format\x18\x1b \x01(\tB\x10\xda\xf3\x18\x04\b\x01\x18\x01\xbaH\x05r\x03\x18\x80\x01H\vR\x0flabelSyncFormat\x88\x01\x01\x12F\n" +
	"\x05files\x18\x1c \x03(\v2\x14.resources.file.FileB\x1a\x9a\x84\x9e\x03\ralias:\"files\"\xbaH\x05\x92\x01\x02\x10\x05R\x05filesB\r\n" +
	"\v_created_atB\r\n" +
	"\v_updated_atB\r\n" +
	"\v_deleted_atB\x0e\n" +
	"\f_descriptionB\r\n" +
	"\v_creator_idB\n" +
	"\n" +
	"\b_creatorB\x13\n" +
	"\x11_discord_settingsB\x10\n" +
	"\x0e_exam_settingsB\a\n" +
	"\x05_examB\t\n" +
	"\a_resultB\n" +
	"\n" +
	"\b_requestB\x14\n" +
	"\x12_label_sync_format\"\xb7\t\n" +
	"\x12QualificationShort\x121\n" +
	"\x02id\x18\x01 \x01(\x03B!\x9a\x84\x9e\x03\x1csql:\"primary_key\" alias:\"id\"R\x02id\x12B\n" +
	"\n" +
	"created_at\x18\x02 \x01(\v2\x1e.resources.timestamp.TimestampH\x00R\tcreatedAt\x88\x01\x01\x12B\n" +
	"\n" +
	"updated_at\x18\x03 \x01(\v2\x1e.resources.timestamp.TimestampH\x01R\tupdatedAt\x88\x01\x01\x12B\n" +
	"\n" +
	"deleted_at\x18\x04 \x01(\v2\x1e.resources.timestamp.TimestampH\x02R\tdeletedAt\x88\x01\x01\x12\x19\n" +
	"\x03job\x18\x05 \x01(\tB\a\xbaH\x04r\x02\x18\x14R\x03job\x12#\n" +
	"\x06weight\x18\x06 \x01(\rB\v\xbaH\b*\x06\x10\xff\xff\xff\xff\x0fR\x06weight\x12\x16\n" +
	"\x06closed\x18\a \x01(\bR\x06closed\x12\x14\n" +
	"\x05draft\x18\b \x01(\bR\x05draft\x12\x16\n" +
	"\x06public\x18\t \x01(\bR\x06public\x123\n" +
	"\fabbreviation\x18\n" +
	" \x01(\tB\x0f\xda\xf3\x18\x04\b\x01\x18\x01\xbaH\x04r\x02\x18\x14R\fabbreviation\x12&\n" +
	"\x05title\x18\v \x01(\tB\x10\xda\xf3\x18\x02\b\x01\xbaH\ar\x05\x10\x03\x18\x80\bR\x05title\x127\n" +
	"\vdescription\x18\f \x01(\tB\x10\xda\xf3\x18\x04\b\x01\x18\x01\xbaH\x05r\x03\x18\x80\x04H\x03R\vdescription\x88\x01\x01\x12+\n" +
	"\n" +
	"creator_id\x18\x0e \x01(\x05B\a\xbaH\x04\x1a\x02 \x00H\x04R\tcreatorId\x88\x01\x01\x12U\n" +
	"\acreator\x18\x0f \x01(\v2 .resources.users.short.UserShortB\x14\x9a\x84\x9e\x03\x0falias:\"creator\"H\x05R\acreator\x88\x01\x01\x12(\n" +
	"\vcreator_job\x18\x10 \x01(\tB\a\xbaH\x04r\x02\x18\x14R\n" +
	"creatorJob\x12V\n" +
	"\frequirements\x18\x12 \x03(\v22.resources.qualifications.QualificationRequirementR\frequirements\x12[\n" +
	"\texam_mode\x18\x15 \x01(\x0e24.resources.qualifications.exam.QualificationExamModeB\b\xbaH\x05\x82\x01\x02\x10\x01R\bexamMode\x12b\n" +
	"\rexam_settings\x18\x16 \x01(\v28.resources.qualifications.exam.QualificationExamSettingsH\x06R\fexamSettings\x88\x01\x01\x12J\n" +
	"\x06result\x18\x18 \x01(\v2-.resources.qualifications.QualificationResultH\aR\x06result\x88\x01\x01B\r\n" +
	"\v_created_atB\r\n" +
	"\v_updated_atB\r\n" +
	"\v_deleted_atB\x0e\n" +
	"\f_descriptionB\r\n" +
	"\v_creator_idB\n" +
	"\n" +
	"\b_creatorB\x10\n" +
	"\x0e_exam_settingsB\t\n" +
	"\a_result\"\xa5\x03\n" +
	"\x18QualificationRequirement\x121\n" +
	"\x02id\x18\x01 \x01(\x03B!\x9a\x84\x9e\x03\x1csql:\"primary_key\" alias:\"id\"R\x02id\x12B\n" +
	"\n" +
	"created_at\x18\x02 \x01(\v2\x1e.resources.timestamp.TimestampH\x00R\tcreatedAt\x88\x01\x01\x12)\n" +
	"\x10qualification_id\x18\x03 \x01(\x03R\x0fqualificationId\x126\n" +
	"\x17target_qualification_id\x18\x04 \x01(\x03R\x15targetQualificationId\x12\x86\x01\n" +
	"\x14target_qualification\x18\x05 \x01(\v2,.resources.qualifications.QualificationShortB \x9a\x84\x9e\x03\x1balias:\"targetqualification\"H\x01R\x13targetQualification\x88\x01\x01B\r\n" +
	"\v_created_atB\x17\n" +
	"\x15_target_qualification\"\x9e\x01\n" +
	"\x1cQualificationDiscordSettings\x12)\n" +
	"\trole_name\x18\x01 \x01(\tB\a\xbaH\x04r\x02\x18@H\x00R\broleName\x88\x01\x01\x12-\n" +
	"\vrole_format\x18\x02 \x01(\tB\a\xbaH\x04r\x02\x18@H\x01R\n" +
	"roleFormat\x88\x01\x01:\x06\xe2\xf3\x18\x02\b\x01B\f\n" +
	"\n" +
	"_role_nameB\x0e\n" +
	"\f_role_format\"\xb6\b\n" +
	"\x14QualificationRequest\x12B\n" +
	"\n" +
	"created_at\x18\x01 \x01(\v2\x1e.resources.timestamp.TimestampH\x00R\tcreatedAt\x88\x01\x01\x12B\n" +
	"\n" +
	"deleted_at\x18\x02 \x01(\v2\x1e.resources.timestamp.TimestampH\x01R\tdeletedAt\x88\x01\x01\x12Z\n" +
	"\x10qualification_id\x18\x03 \x01(\x03B/\x9a\x84\x9e\x03*sql:\"primary_key\" alias:\"qualification_id\"R\x0fqualificationId\x12W\n" +
	"\rqualification\x18\x04 \x01(\v2,.resources.qualifications.QualificationShortH\x02R\rqualification\x88\x01\x01\x126\n" +
	"\auser_id\x18\x05 \x01(\x05B\x1d\x9a\x84\x9e\x03\x11sql:\"primary_key\"\xbaH\x04\x1a\x02(\x00R\x06userId\x12G\n" +
	"\x04user\x18\x06 \x01(\v2 .resources.users.short.UserShortB\x11\x9a\x84\x9e\x03\falias:\"user\"R\x04user\x128\n" +
	"\fuser_comment\x18\a \x01(\tB\x10\xda\xf3\x18\x04\b\x01\x18\x01\xbaH\x05r\x03\x18\x80\x04H\x03R\vuserComment\x88\x01\x01\x12N\n" +
	"\x06status\x18\b \x01(\x0e2'.resources.qualifications.RequestStatusB\b\xbaH\x05\x82\x01\x02\x10\x01H\x04R\x06status\x88\x01\x01\x12D\n" +
	"\vapproved_at\x18\t \x01(\v2\x1e.resources.timestamp.TimestampH\x05R\n" +
	"approvedAt\x88\x01\x01\x12@\n" +
	"\x10approver_comment\x18\n" +
	" \x01(\tB\x10\xda\xf3\x18\x04\b\x01\x18\x01\xbaH\x05r\x03\x18\x80\x04H\x06R\x0fapproverComment\x88\x01\x01\x12-\n" +
	"\vapprover_id\x18\v \x01(\x05B\a\xbaH\x04\x1a\x02 \x00H\aR\n" +
	"approverId\x88\x01\x01\x12X\n" +
	"\bapprover\x18\f \x01(\v2 .resources.users.short.UserShortB\x15\x9a\x84\x9e\x03\x10alias:\"approver\"H\bR\bapprover\x88\x01\x01\x12&\n" +
	"\fapprover_job\x18\r \x01(\tH\tR\vapproverJob\x88\x01\x01B\r\n" +
	"\v_created_atB\r\n" +
	"\v_deleted_atB\x10\n" +
	"\x0e_qualificationB\x0f\n" +
	"\r_user_commentB\t\n" +
	"\a_statusB\x0e\n" +
	"\f_approved_atB\x13\n" +
	"\x11_approver_commentB\x0e\n" +
	"\f_approver_idB\v\n" +
	"\t_approverB\x0f\n" +
	"\r_approver_job\"\xbf\x06\n" +
	"\x13QualificationResult\x121\n" +
	"\x02id\x18\x01 \x01(\x03B!\x9a\x84\x9e\x03\x1csql:\"primary_key\" alias:\"id\"R\x02id\x12B\n" +
	"\n" +
	"created_at\x18\x02 \x01(\v2\x1e.resources.timestamp.TimestampH\x00R\tcreatedAt\x88\x01\x01\x12B\n" +
	"\n" +
	"deleted_at\x18\x03 \x01(\v2\x1e.resources.timestamp.TimestampH\x01R\tdeletedAt\x88\x01\x01\x12)\n" +
	"\x10qualification_id\x18\x04 \x01(\x03R\x0fqualificationId\x12W\n" +
	"\rqualification\x18\x05 \x01(\v2,.resources.qualifications.QualificationShortH\x02R\rqualification\x88\x01\x01\x12 \n" +
	"\auser_id\x18\x06 \x01(\x05B\a\xbaH\x04\x1a\x02(\x00R\x06userId\x12G\n" +
	"\x04user\x18\a \x01(\v2 .resources.users.short.UserShortB\x11\x9a\x84\x9e\x03\falias:\"user\"R\x04user\x12H\n" +
	"\x06status\x18\b \x01(\x0e2&.resources.qualifications.ResultStatusB\b\xbaH\x05\x82\x01\x02\x10\x01R\x06status\x12*\n" +
	"\x05score\x18\t \x01(\x02B\x0f\xbaH\f\n" +
	"\n" +
	"\x1d\x00\x00zD-\x00\x00\x00\x00H\x03R\x05score\x88\x01\x01\x12*\n" +
	"\asummary\x18\n" +
	" \x01(\tB\x10\xda\xf3\x18\x04\b\x01\x18\x01\xbaH\x05r\x03\x18\x80\x04R\asummary\x12&\n" +
	"\n" +
	"creator_id\x18\v \x01(\x05B\a\xbaH\x04\x1a\x02 \x00R\tcreatorId\x12P\n" +
	"\acreator\x18\f \x01(\v2 .resources.users.short.UserShortB\x14\x9a\x84\x9e\x03\x0falias:\"creator\"R\acreator\x12(\n" +
	"\vcreator_job\x18\r \x01(\tB\a\xbaH\x04r\x02\x18\x14R\n" +
	"creatorJobB\r\n" +
	"\v_created_atB\r\n" +
	"\v_deleted_atB\x10\n" +
	"\x0e_qualificationB\b\n" +
	"\x06_score*\xe3\x01\n" +
	"\rRequestStatus\x12\x1e\n" +
	"\x1aREQUEST_STATUS_UNSPECIFIED\x10\x00\x12\x1a\n" +
	"\x16REQUEST_STATUS_PENDING\x10\x01\x12\x19\n" +
	"\x15REQUEST_STATUS_DENIED\x10\x02\x12\x1b\n" +
	"\x17REQUEST_STATUS_ACCEPTED\x10\x03\x12\x1f\n" +
	"\x1bREQUEST_STATUS_EXAM_STARTED\x10\x04\x12\x1f\n" +
	"\x1bREQUEST_STATUS_EXAM_GRADING\x10\x05\x12\x1c\n" +
	"\x18REQUEST_STATUS_COMPLETED\x10\x06*\x80\x01\n" +
	"\fResultStatus\x12\x1d\n" +
	"\x19RESULT_STATUS_UNSPECIFIED\x10\x00\x12\x19\n" +
	"\x15RESULT_STATUS_PENDING\x10\x01\x12\x18\n" +
	"\x14RESULT_STATUS_FAILED\x10\x02\x12\x1c\n" +
	"\x18RESULT_STATUS_SUCCESSFUL\x10\x03B[ZYgithub.com/fivenet-app/fivenet/v2026/gen/go/proto/resources/qualifications;qualificationsb\x06proto3"

var file_resources_qualifications_qualifications_proto_enumTypes = make([]protoimpl.EnumInfo, 2)
var file_resources_qualifications_qualifications_proto_msgTypes = make([]protoimpl.MessageInfo, 6)
var file_resources_qualifications_qualifications_proto_goTypes = []any{
	(RequestStatus)(0),                     // 0: resources.qualifications.RequestStatus
	(ResultStatus)(0),                      // 1: resources.qualifications.ResultStatus
	(*Qualification)(nil),                  // 2: resources.qualifications.Qualification
	(*QualificationShort)(nil),             // 3: resources.qualifications.QualificationShort
	(*QualificationRequirement)(nil),       // 4: resources.qualifications.QualificationRequirement
	(*QualificationDiscordSettings)(nil),   // 5: resources.qualifications.QualificationDiscordSettings
	(*QualificationRequest)(nil),           // 6: resources.qualifications.QualificationRequest
	(*QualificationResult)(nil),            // 7: resources.qualifications.QualificationResult
	(*timestamp.Timestamp)(nil),            // 8: resources.timestamp.Timestamp
	(*content.Content)(nil),                // 9: resources.common.content.Content
	(*short.UserShort)(nil),                // 10: resources.users.short.UserShort
	(*access.QualificationAccess)(nil),     // 11: resources.qualifications.access.QualificationAccess
	(exam.QualificationExamMode)(0),        // 12: resources.qualifications.exam.QualificationExamMode
	(*exam.QualificationExamSettings)(nil), // 13: resources.qualifications.exam.QualificationExamSettings
	(*exam.ExamQuestions)(nil),             // 14: resources.qualifications.exam.ExamQuestions
	(*file.File)(nil),                      // 15: resources.file.File
}
var file_resources_qualifications_qualifications_proto_depIdxs = []int32{
	8,  // 0: resources.qualifications.Qualification.created_at:type_name -> resources.timestamp.Timestamp
	8,  // 1: resources.qualifications.Qualification.updated_at:type_name -> resources.timestamp.Timestamp
	8,  // 2: resources.qualifications.Qualification.deleted_at:type_name -> resources.timestamp.Timestamp
	9,  // 3: resources.qualifications.Qualification.content:type_name -> resources.common.content.Content
	10, // 4: resources.qualifications.Qualification.creator:type_name -> resources.users.short.UserShort
	11, // 5: resources.qualifications.Qualification.access:type_name -> resources.qualifications.access.QualificationAccess
	4,  // 6: resources.qualifications.Qualification.requirements:type_name -> resources.qualifications.QualificationRequirement
	5,  // 7: resources.qualifications.Qualification.discord_settings:type_name -> resources.qualifications.QualificationDiscordSettings
	12, // 8: resources.qualifications.Qualification.exam_mode:type_name -> resources.qualifications.exam.QualificationExamMode
	13, // 9: resources.qualifications.Qualification.exam_settings:type_name -> resources.qualifications.exam.QualificationExamSettings
	14, // 10: resources.qualifications.Qualification.exam:type_name -> resources.qualifications.exam.ExamQuestions
	7,  // 11: resources.qualifications.Qualification.result:type_name -> resources.qualifications.QualificationResult
	6,  // 12: resources.qualifications.Qualification.request:type_name -> resources.qualifications.QualificationRequest
	15, // 13: resources.qualifications.Qualification.files:type_name -> resources.file.File
	8,  // 14: resources.qualifications.QualificationShort.created_at:type_name -> resources.timestamp.Timestamp
	8,  // 15: resources.qualifications.QualificationShort.updated_at:type_name -> resources.timestamp.Timestamp
	8,  // 16: resources.qualifications.QualificationShort.deleted_at:type_name -> resources.timestamp.Timestamp
	10, // 17: resources.qualifications.QualificationShort.creator:type_name -> resources.users.short.UserShort
	4,  // 18: resources.qualifications.QualificationShort.requirements:type_name -> resources.qualifications.QualificationRequirement
	12, // 19: resources.qualifications.QualificationShort.exam_mode:type_name -> resources.qualifications.exam.QualificationExamMode
	13, // 20: resources.qualifications.QualificationShort.exam_settings:type_name -> resources.qualifications.exam.QualificationExamSettings
	7,  // 21: resources.qualifications.QualificationShort.result:type_name -> resources.qualifications.QualificationResult
	8,  // 22: resources.qualifications.QualificationRequirement.created_at:type_name -> resources.timestamp.Timestamp
	3,  // 23: resources.qualifications.QualificationRequirement.target_qualification:type_name -> resources.qualifications.QualificationShort
	8,  // 24: resources.qualifications.QualificationRequest.created_at:type_name -> resources.timestamp.Timestamp
	8,  // 25: resources.qualifications.QualificationRequest.deleted_at:type_name -> resources.timestamp.Timestamp
	3,  // 26: resources.qualifications.QualificationRequest.qualification:type_name -> resources.qualifications.QualificationShort
	10, // 27: resources.qualifications.QualificationRequest.user:type_name -> resources.users.short.UserShort
	0,  // 28: resources.qualifications.QualificationRequest.status:type_name -> resources.qualifications.RequestStatus
	8,  // 29: resources.qualifications.QualificationRequest.approved_at:type_name -> resources.timestamp.Timestamp
	10, // 30: resources.qualifications.QualificationRequest.approver:type_name -> resources.users.short.UserShort
	8,  // 31: resources.qualifications.QualificationResult.created_at:type_name -> resources.timestamp.Timestamp
	8,  // 32: resources.qualifications.QualificationResult.deleted_at:type_name -> resources.timestamp.Timestamp
	3,  // 33: resources.qualifications.QualificationResult.qualification:type_name -> resources.qualifications.QualificationShort
	10, // 34: resources.qualifications.QualificationResult.user:type_name -> resources.users.short.UserShort
	1,  // 35: resources.qualifications.QualificationResult.status:type_name -> resources.qualifications.ResultStatus
	10, // 36: resources.qualifications.QualificationResult.creator:type_name -> resources.users.short.UserShort
	37, // [37:37] is the sub-list for method output_type
	37, // [37:37] is the sub-list for method input_type
	37, // [37:37] is the sub-list for extension type_name
	37, // [37:37] is the sub-list for extension extendee
	0,  // [0:37] is the sub-list for field type_name
}

func init() { file_resources_qualifications_qualifications_proto_init() }
func file_resources_qualifications_qualifications_proto_init() {
	if File_resources_qualifications_qualifications_proto != nil {
		return
	}
	file_resources_qualifications_qualifications_proto_msgTypes[0].OneofWrappers = []any{}
	file_resources_qualifications_qualifications_proto_msgTypes[1].OneofWrappers = []any{}
	file_resources_qualifications_qualifications_proto_msgTypes[2].OneofWrappers = []any{}
	file_resources_qualifications_qualifications_proto_msgTypes[3].OneofWrappers = []any{}
	file_resources_qualifications_qualifications_proto_msgTypes[4].OneofWrappers = []any{}
	file_resources_qualifications_qualifications_proto_msgTypes[5].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_resources_qualifications_qualifications_proto_rawDesc), len(file_resources_qualifications_qualifications_proto_rawDesc)),
			NumEnums:      2,
			NumMessages:   6,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_resources_qualifications_qualifications_proto_goTypes,
		DependencyIndexes: file_resources_qualifications_qualifications_proto_depIdxs,
		EnumInfos:         file_resources_qualifications_qualifications_proto_enumTypes,
		MessageInfos:      file_resources_qualifications_qualifications_proto_msgTypes,
	}.Build()
	File_resources_qualifications_qualifications_proto = out.File
	file_resources_qualifications_qualifications_proto_goTypes = nil
	file_resources_qualifications_qualifications_proto_depIdxs = nil
}
