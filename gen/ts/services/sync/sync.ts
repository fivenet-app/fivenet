...
class ClientSyncState$Type extends MessageType<ClientSyncState> {
    ...
    create(value?: PartialMessage<ClientSyncState>): ClientSyncState {
        ...
        message.tables = [];
        if (value !== undefined)
            reflectionMergePartial<ClientSyncState>(this, message, value);
        return message;
    }
    internalBinaryRead(reader: IBinaryReader, length: number, options: BinaryReadOptions, target?: ClientSyncState): ClientSyncState {
        let message = target ?? this.create(), end = reader.pos + length;
        while (reader.pos < end) {
            let [fieldNo, wireType] = reader.tag();
            switch (fieldNo) {
