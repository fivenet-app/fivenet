...
async function assignUnit(id: number, job: string): Promise<void> {
    try {
        const toAdd: number[] = [];
...
const onSubmitThrottle = useThrottleFn(async () => {
    ...
    await assignUnit(props.unit.id, props.unit.job).finally(() => useTimeoutFn(() => (canSubmit.value = true), 400));
}, 1000);
</script>
