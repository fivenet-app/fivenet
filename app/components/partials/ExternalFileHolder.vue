...
const { data: content, status, error, refresh } = useLazyFetch<string>(props.path);
</script>

<template>
    <div class="max-w-full px-4 text-black dark:text-white">
        <p v-if="isRequestPending(status) || !content" class="text-lg">{{ $t('common.loading', $t('common.licenses', 1)) }}</p>
        ...
        <p v-else class="text-lg">
            ...
            <UButton :label="$t('common.retry')" icon="i-mdi-refresh" @click="() => refresh()" />
        </p>
    </div>
</template>
