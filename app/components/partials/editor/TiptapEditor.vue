...
const imageModal = overlay.create(TiptapEditorImageModal, {
    props: {
        editor: editor.value!,
    },
});

const sourceCodeModal = overlay.create(TiptapEditorSourceCodeModal, {
    props: {
...
<template>
    <div class="relative flex flex-col border border-neutral-100 dark:border-neutral-800" :class="rounded">
            ...
            <div class="flex snap-x flex-wrap gap-1">
                ...
                <template v-if="!commentMode">
                    ...
                    <UTooltip :text="$t('components.partials.TipTapEditor.source_code')" :popper="{ placement: 'top' }">
                        <UButton color="neutral" variant="ghost" icon="i-mdi-file-code" @click="sourceCodeModal.open()" />
                    </UTooltip>
                </template>
            </div>
        </div>
