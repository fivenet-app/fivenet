...
import type { ListAccountsResponse } from '~~/gen/ts/services/rector/accounts';
...
const {
    ...
    refresh,
    error,
} = useLazyAsyncData(`rector-accounts-${sort.value.column}:${sort.value.direction}-${page.value}`, () => listAccounts());

async function listAccounts(): Promise<ListAccountsResponse> {
    try {
        const call = $grpc.rector.rectorAccounts.listAccounts({
            pagination: {
                offset: offset.value,
            },
            sort: sort.value,
...
async function deleteAccount(id: number): Promise<void> {
    try {
        const call = $grpc.rector.rectorAccounts.deleteAccount({
            id,
        });
        await call;
...
<template>
    <template v-if="streamerMode">
        <UDashboardNavbar :title="$t('pages.rector.accounts.title')">
            <template #right>
