...
CREATE TABLE
    IF NOT EXISTS `fivenet_jobs_conduct` (
        ...
        CONSTRAINT `fk_fivenet_jobs_conduct_target_user_id` FOREIGN KEY (`target_user_id`) REFERENCES `{{.UsersTableName}}` (`id`)
    ON DELETE CASCADE ON UPDATE CASCADE,
        CONSTRAINT `fk_fivenet_jobs_conduct_creator_id` FOREIGN KEY (`creator_id`) REFERENCES `{{.UsersTableName}}` (`id`)
...
CREATE TABLE
    IF NOT EXISTS `fivenet_jobs_timeclock` (
        ...
        CONSTRAINT `fk_fivenet_jobs_timeclock_user_id` FOREIGN KEY (`user_id`) REFERENCES `{{.UsersTableName}}` (`id`)
    ON DELETE CASCADE ON UPDATE CASCADE
    ) ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `fivenet_jobs_user_props` (
  ...
  CONSTRAINT `fk_fivenet_jobs_user_props_user_id` FOREIGN KEY (`user_id`) REFERENCES `{{.UsersTableName}}` (`id`)
    ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS `fivenet_jobs_user_activity` (
  ...
  CONSTRAINT `fk_fivenet_jobs_user_activity_source_user_id` FOREIGN KEY (`source_user_id`) REFERENCES `{{.UsersTableName}}` (`id`)
    ON DELETE SET NULL ON UPDATE SET NULL,
  CONSTRAINT `fk_fivenet_jobs_user_activity_target_user_id` FOREIGN KEY (`target_user_id`) REFERENCES `{{.UsersTableName}}` (`id`)
    ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
