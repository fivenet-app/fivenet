...
CREATE TABLE IF NOT EXISTS `fivenet_user_citizen_attributes` (
  ...
  CONSTRAINT `fk_fivenet_user_citizen_attributes_user_id` FOREIGN KEY (`user_id`) REFERENCES `{{.UsersTableName}}` (`id`)
    ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_fivenet_user_citizen_attributes_attribute_id` FOREIGN KEY (`attribute_id`) REFERENCES `fivenet_job_citizen_attributes` (`id`)
    ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
