...
<template>
    <div class="py-2 mt-5 max-w-5xl mx-auto">
        ...
        <DataPendingBlock v-if="pending" :message="$t('common.loading', ['account info'])" />
        <DataErrorBlock v-else-if="error" :title="$t('common.unable_to_load', ['account info'])" :retry="refresh" />
        <button v-else-if="!account" type="button"
            ...
            <span class="block mt-2 text-sm font-semibold">
                {{ $t('common.not_found', ['account data']) }}
            </span>
        </button>
        <div v-else>
