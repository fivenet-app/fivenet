...
import { QualificationRequest, ResultStatus } from '~~/gen/ts/resources/qualifications/qualifications';
        ...
        <div class="flex min-w-0 gap-x-4">
            <div class="min-w-0 flex-auto">
                <p class="text-sm font-semibold leading-6 text-gray-100">
                    <NuxtLink :to="{ name: 'jobs-qualifications-id', params: { id: request.id } }">
                        <span class="absolute inset-x-0 -top-px bottom-0" />
                        {{ request.abbreviation }}: {{ request.title }}
                    </NuxtLink>
                </p>
                <p class="mt-1 flex text-xs leading-5 text-gray-300">
                    <span class="inline-flex gap-1">
                        <span class="font-semibold">{{ $t('common.score') }}: {{ request.result?.score }}</span>
                        <span v-if="request.summary"> ({{ $t('common.summary') }}: {{ request.result?.summary }})</span>
                    </span>
                </p>
            </div>
        </div>
