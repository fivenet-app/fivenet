...
const { data, pending, refresh, error } = useLazyAsyncData(`qualifications-requests`, () => listQualificationsRequests());

async function listQualificationsRequests(): Promise<ListQualificationRequestsResponse> {
    try {
        ...
        <div class="px-1 sm:px-2 lg:px-4">
            ...
            <template v-else>
                <ul role="list" class="divide-y divide-gray-100">
                    <QualificationsRequestsListEntry v-for="result in data?.requests" :key="result.id" :request="result" />
                </ul>
            </template>
        </div>
