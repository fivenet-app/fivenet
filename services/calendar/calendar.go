...
func (s *Server) ListCalendars(ctx context.Context, req *pbcalendar.ListCalendarsRequest) (*pbcalendar.ListCalendarsResponse, error) {
	...
	stmt := tCalendar.
		SELECT(
			...
			tCalendarSubs.Muted,
			tCAccess.ID.AS("citizen_access.id"),
			tCAccess.TargetID.AS("citizen_access.target_id"),
			tCAccess.UserID.AS("citizen_access.user_id"),
			tCAccess.Access.AS("citizen_access.access"),
