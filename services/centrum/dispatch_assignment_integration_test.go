...
func newDispatchAssignmentFixture(t *testing.T) (*Server, *sql.DB, int64, int64) {
	...
	ctx := auth.ContextWithUserInfo(t.Context(), &userinfo.UserInfo{UserId: 1, Job: "ambulance", JobGrade: 1})
	unit := createUnitForTest(t, srv, ctx, "Assignment-Test")
	require.NoError(t, srv.units.UpdateUnitAssignments(ctx, "ambulance", new(int32), unit.GetId(), []int32{1}, nil))
...
func takeDispatchForTest(t *testing.T, srv *Server, dispatchID int64, resp centrumdispatches.TakeDispatchResp) error {
	t.Helper()
	ctx := auth.ContextWithUserInfo(t.Context(), &userinfo.UserInfo{UserId: 1, Job: "ambulance", JobGrade: 1})
...
func TestAssignDispatchCreatesThirtySecondPendingAssignment(t *testing.T) {
	...
	_, err := srv.AssignDispatch(ctx, &pbcentrum.AssignDispatchRequest{DispatchId: dispatchID, ToAdd: []int64{unitID}})
	require.NoError(t, err)
...
func TestTakeDispatchAcceptanceCancelsTimerAndClearsExpiry(t *testing.T) {
	...
	require.NoError(t, takeDispatchForTest(t, srv, dispatchID, centrumdispatches.TakeDispatchResp_TAKE_DISPATCH_RESP_ACCEPTED))
	require.False(t, assignmentExpiryForTest(t, db, dispatchID, unitID).Valid)
	dsp, err := srv.dispatches.Get(ctx, dispatchID)
	require.NoError(t, err)
	require.Nil(t, dsp.GetUnits()[0].GetExpiresAt())
...
func TestTakeDispatchDeclineRemovesAssignmentAndTimer(t *testing.T) {
	...
	require.NoError(t, takeDispatchForTest(t, srv, dispatchID, centrumdispatches.TakeDispatchResp_TAKE_DISPATCH_RESP_DECLINED))
	var count int
	require.NoError(t, db.QueryRow("SELECT COUNT(*) FROM fivenet_centrum_dispatches_asgmts WHERE dispatch_id = ? AND unit_id = ?", dispatchID, unitID).Scan(&count))
...
func TestTakeDispatchTwoSecondGraceBoundary(t *testing.T) {
	t.Run("within grace", func(t *testing.T) {
		...
		_, err := srv.AssignDispatch(ctx, &pbcentrum.AssignDispatchRequest{DispatchId: dispatchID, ToAdd: []int64{unitID}})
		require.NoError(t, err)
		_, err = db.Exec("UPDATE fivenet_centrum_dispatches_asgmts SET expires_at = DATE_SUB(CURRENT_TIMESTAMP(), INTERVAL 1 SECOND) WHERE dispatch_id = ? AND unit_id = ?", dispatchID, unitID)
		require.NoError(t, err)
		require.NoError(t, takeDispatchForTest(t, srv, dispatchID, centrumdispatches.TakeDispatchResp_TAKE_DISPATCH_RESP_ACCEPTED))
		require.False(t, assignmentExpiryForTest(t, db, dispatchID, unitID).Valid)
	})
	...
	t.Run("after grace", func(t *testing.T) {
		...
		_, err := srv.AssignDispatch(ctx, &pbcentrum.AssignDispatchRequest{DispatchId: dispatchID, ToAdd: []int64{unitID}})
		require.NoError(t, err)
		_, err = db.Exec("UPDATE fivenet_centrum_dispatches_asgmts SET expires_at = DATE_SUB(CURRENT_TIMESTAMP(), INTERVAL 3 SECOND) WHERE dispatch_id = ? AND unit_id = ?", dispatchID, unitID)
		require.NoError(t, err)
...
func TestEmptyUnitCleanupRemovesAssignmentAndTimer(t *testing.T) {
	...
	require.NoError(t, srv.units.RemoveUnitAssignments(ctx, "ambulance", new(int32), unitID, []int32{1}))
	require.NoError(t, srv.dispatches.UpdateAssignments(ctx, new("ambulance"), nil, dispatchID, nil, []int64{unitID}, time.Time{}))
