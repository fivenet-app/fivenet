	...
	g.Go(func() error {
		...
		for {
			...
			case change, ok := <-userInfoChanges:
				...
				// The event is only a resync signal. It can have been buffered before
				// the initial database refresh or durably replayed after a newer
				// change, so its payload is not authoritative.
